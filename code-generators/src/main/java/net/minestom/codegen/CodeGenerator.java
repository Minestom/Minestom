...
public class CodeGenerator {
    ...
    public void generateKeys(InputStream resourceFile, String packageName, String typeName, String generatedName, boolean publicKeys) {
        ...
        TypeSpec.Builder blockConstantsClass = TypeSpec.interfaceBuilder(materialsCN)
                ...
                .addAnnotation(AnnotationSpec.builder(SuppressWarnings.class).addMember("value", "$S", "unused").build())
                .addJavadoc("Code autogenerated, do not edit!");
        if (publicKeys) blockConstantsClass.addModifiers(Modifier.PUBLIC);
    ...
    public void generateTags(InputStream resourceFile, String packageName, String typeName, String generatedName) {
        if (resourceFile == null) {
            // We won't throw here as some tags are optional
            LOGGER.warn("Failed to find (tags) resource file for {}", typeName);
            return;
        }

        final JsonObject json = GSON.fromJson(new InputStreamReader(resourceFile), JsonObject.class);
        Objects.requireNonNull(json, "Failed to parse JSON for " + typeName);
        ...
        writeFiles(
                List.of(JavaFile.builder(packageName, registryTagInterface.build())
                        .indent("    ")
                        .skipJavaLangImports(true)
