...
class ItemMetaBuilderRegressions {
    private class BasicMetaBuilder : ItemMetaBuilder() {
        override fun build(): ItemMeta {
            return object : ItemMeta(this) {}
        }

        override fun read(nbtCompound: NBTCompound) {
            // don't care
        }

        public override fun handleCompound(key: String, consumer: Consumer<MutableNBTCompound>) {
            super.handleCompound(key, consumer)
        }
    }

    @Test
    fun handleCompoundShouldUsePreviousValue() {
        val builder = BasicMetaBuilder()
        builder.handleCompound("test") { nbt: MutableNBTCompound -> nbt.setString("my_key", "AAA") }
        var nbt = builder.build().toNBT().getCompound("test")
        Assertions.assertTrue(nbt!!.contains("my_key"))
        Assertions.assertEquals(1, nbt.getSize())
        Assertions.assertTrue(nbt["my_key"] is NBTString)
        Assertions.assertEquals("AAA", nbt.getString("my_key"))
        builder.handleCompound("test") { n: MutableNBTCompound -> n.setString("my_other_key", "BBB") }
        nbt = builder.build().toNBT().getCompound("test")
        Assertions.assertTrue(nbt!!.contains("my_key"))
        Assertions.assertTrue(nbt.contains("my_other_key"))
        Assertions.assertEquals(2, nbt.getSize())
        Assertions.assertTrue(nbt["my_key"] is NBTString)
        Assertions.assertTrue(nbt["my_other_key"] is NBTString)
        Assertions.assertEquals("AAA", nbt.getString("my_key"))
        Assertions.assertEquals("BBB", nbt.getString("my_other_key"))
    }

    @Test
    fun clearingShouldRemoveData() {
        val builder = BasicMetaBuilder()
        builder.handleCompound("test") { nbt: MutableNBTCompound -> nbt.setString("my_key", "AAA") }
        val nbt = builder.build().toNBT().getCompound("test")
        Assertions.assertTrue(nbt!!.contains("my_key"))
        Assertions.assertEquals(1, nbt.getSize())
        Assertions.assertTrue(nbt["my_key"] is NBTString)
        Assertions.assertEquals("AAA", nbt.getString("my_key"))
        builder.handleCompound("test") { n: MutableNBTCompound -> n.clear() }
        val rootNBT = builder.build().toNBT()
        Assertions.assertFalse(rootNBT.contains("test"))
        Assertions.assertEquals(0, rootNBT.getSize())
    }
}