...
class PooledBuffersTest {
    @Test
    fun pool() {
        val pooledBuffers: MutableSet<BinaryBuffer> = HashSet()
        clear()
        ...
        buffer = get()
        Assertions.assertTrue(pooledBuffers.add(buffer))
        add(buffer)
        Assertions.assertEquals(1, count())
        buffer = get()
        Assertions.assertEquals(0, count())
        Assertions.assertFalse(pooledBuffers.add(buffer))
    }
}