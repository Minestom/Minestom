...
class ServerAddressTest {
    ...
    fun inetAddressTest() {
        ...
        val server = Server(PacketProcessor())
        server.init(address)
        Assertions.assertSame(address, server.socketAddress())
        Assertions.assertEquals(address.hostString, server.address)
        Assertions.assertEquals(address.port, server.port)
    ...
    fun unixAddressTest() {
        val address = UnixDomainSocketAddress.of("minestom.sock")
        val server = Server(PacketProcessor())
        server.init(address)
        Assertions.assertTrue(Files.exists(address.path))
        Assertions.assertSame(address, server.socketAddress())
        Assertions.assertEquals("unix://" + address.path, server.address)
        Assertions.assertEquals(0, server.port)
        Assertions.assertDoesNotThrow { server.start() }
        Assertions.assertDoesNotThrow { server.stop() }
        Assertions.assertFalse(Files.exists(address.path), "The socket file should be deleted")
    }
}