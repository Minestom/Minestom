...
class SocketReadTest {
    ...
    fun complete(compressed: Boolean) {
        val packet = ClientPluginMessagePacket("channel", ByteArray(2000))
        val buffer = packetBuffer()
        PacketUtils.writeFramedPacket(buffer, 0x0A, packet, if (compressed) 256 else 0)
        val wrapper = wrap(buffer)
        wrapper.reset(0, buffer.position())
        val packets: MutableList<Pair<Int, ByteBuffer>> = ArrayList()
        val remaining = PacketUtils.readPackets(
            wrapper, compressed
        ) { integer: Int, payload: ByteBuffer -> packets.add(Pair.of(integer, payload)) }
        Assertions.assertNull(remaining)
        Assertions.assertEquals(1, packets.size)
        val rawPacket = packets[0]
        Assertions.assertEquals(0x0A, rawPacket.left())
        val readPacket = ClientPluginMessagePacket(BinaryReader(rawPacket.right()))
        Assertions.assertEquals("channel", readPacket.channel())
        Assertions.assertEquals(2000, readPacket.data().size)
    }
    ...
    fun completeTwo(compressed: Boolean) {
        val packet = ClientPluginMessagePacket("channel", ByteArray(2000))
        val buffer = packetBuffer()
        PacketUtils.writeFramedPacket(buffer, 0x0A, packet, if (compressed) 256 else 0)
        PacketUtils.writeFramedPacket(buffer, 0x0A, packet, if (compressed) 256 else 0)
    ...
    fun insufficientLength(compressed: Boolean) {
        ...
        Assertions.assertEquals(getVarIntSize(200), remaining!!.readableBytes())
        Assertions.assertEquals(1, packets.size)
        val rawPacket = packets[0]
        Assertions.assertEquals(0x0A, rawPacket.left())
        val readPacket = ClientPluginMessagePacket(BinaryReader(rawPacket.right()))
    ...
    fun incomplete(compressed: Boolean) {
        // Write a complete packet and incomplete var-int length for the next packet
        val packet = ClientPluginMessagePacket("channel", ByteArray(2000))
        val buffer = packetBuffer()
        PacketUtils.writeFramedPacket(buffer, 0x0A, packet, if (compressed) 256 else 0)
        buffer.put(-85.toByte()) // incomplete var-int length
        val wrapper = wrap(buffer)
        wrapper.reset(0, buffer.position())
        val packets: MutableList<Pair<Int, ByteBuffer>> = ArrayList()
        val remaining = PacketUtils.readPackets(
        ...
        Assertions.assertNotNull(remaining)
        Assertions.assertEquals(1, remaining!!.readableBytes())
        Assertions.assertEquals(1, packets.size)
        val rawPacket = packets[0]
        Assertions.assertEquals(0x0A, rawPacket.left())
        val readPacket = ClientPluginMessagePacket(BinaryReader(rawPacket.right()))
