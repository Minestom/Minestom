...
class CommandConditionTest {
    ...
    fun mainCondition() {
        ...
        Assertions.assertNull(dispatcher.findCommand("name"))
        val sender = Sender()
        val sender2 = Sender()
        val called = AtomicBoolean(false)
        val command1 = Command("name")
        command1.defaultExecutor =
            CommandExecutor { sender1: CommandSender?, context: CommandContext? -> called.set(true) }
        command1.condition = CommandCondition { s: CommandSender, commandString: String? -> s === sender }
    ...
    fun subCondition() {
        ...
        run {
            ...
            sub.defaultExecutor =
                CommandExecutor { sender1: CommandSender?, context: CommandContext? -> called.set(true) }
            sub.condition = CommandCondition { s: CommandSender, commandString: String? -> s === sender }
    ...
    fun subConditionOverride() {
        ...
        run {
            ...
            Assertions.assertFalse(
                called.get(),
                "Subcommand execution should have been cancelled by parent command condition"
            )
        }
    }

    private class Sender : CommandSender {
        override val allPermissions: Set<Permission>
            get() = null

        override fun tagHandler(): TagHandler {
            return null
        }
    }
}