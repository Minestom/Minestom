package net.minestom.serverimport
...
class ServerProcessTest {
    @Test
    fun init() {
        val process = AtomicReference<ServerProcess>()
        Assertions.assertDoesNotThrow { process.set(updateProcess()) }
        Assertions.assertDoesNotThrow { process.get().start(InetSocketAddress("localhost", 25565)) }
        Assertions.assertThrows(Exception::class.java) { process.get().start(InetSocketAddress("localhost", 25566)) }
