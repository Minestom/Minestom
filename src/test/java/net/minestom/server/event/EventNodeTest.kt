...
class EventNodeTest {
    ...
    internal class CancellableTest : CancellableEvent {
    ...
    internal open class Recursive1 : RecursiveEvent
    internal class Recursive2 : Recursive1()
    internal inner class ItemTestEvent : ItemEvent {
        override fun getItemStack(): ItemStack {
            return item
        }
    }

    internal inner class EntityTestEvent : EntityEvent {
    ...
    @Test
    fun testCall() {
        val node = EventNode.all("main")
        val result = AtomicBoolean(false)
        val listener = EventListener.of(
            EventTest::class.java
        ) { eventTest: EventTest? -> result.set(true) }
        node.addListener(listener)
        Assertions.assertFalse(result.get(), "The event should not be called before the call")
        node.call(EventTest())
        Assertions.assertTrue(result.get(), "The event should be called after the call")

        // Test removal
        result.set(false)
        node.removeListener(listener)
        node.call(EventTest())
        Assertions.assertFalse(result.get(), "The event should not be called after the removal")
    }

    @Test
    fun testHandle() {
        val node = EventNode.all("main")
        val handle = node.getHandle(EventTest::class.java)
        Assertions.assertSame(handle, node.getHandle(EventTest::class.java))
        val handle1 = node.getHandle(
            CancellableTest::class.java
        )
        Assertions.assertSame(handle1, node.getHandle(CancellableTest::class.java))
    }
    ...
    fun testCancellable() {
        val node = EventNode.all("main")
        val result = AtomicBoolean(false)
        val listener = EventListener.builder(
            CancellableTest::class.java
        )
            .handler { event: CancellableTest ->
                event.isCancelled = true
                result.set(true)
                Assertions.assertTrue(event.isCancelled, "The event should be cancelled")
            }.build()
        node.addListener(listener)
        node.call(CancellableTest())
        Assertions.assertTrue(result.get(), "The event should be called after the call")

        // Test cancelling
        node.addListener(CancellableTest::class.java) { event: CancellableTest? -> Assertions.fail<Any>("The event must have been cancelled") }
        node.call(CancellableTest())
    }
    ...
    fun recursiveSub() {
        ...
        val listener1 = EventListener.of(
            Recursive1::class.java
        ) { event: Recursive1? -> result1.set(true) }
        val listener2 = EventListener.of(
            Recursive2::class.java
        ) { event: Recursive2? -> result2.set(true) }
        node.addListener(listener1)
        node.addListener(listener2)
        node.call(Recursive2())
        Assertions.assertTrue(result2.get(), "Recursive2 should have been called directly")
        Assertions.assertTrue(result1.get(), "Recursive1 should be called due to the RecursiveEvent interface")
        ...
        node.call(Recursive2())
        Assertions.assertFalse(result2.get(), "There is no listener for Recursive2")
        Assertions.assertTrue(result1.get(), "Recursive1 should be called due to the RecursiveEvent interface")
    }
    ...
    fun testChildren() {
        ...
        val child1 = EventNode.all("child1").setPriority(1)
            .addListener(EventTest::class.java) { eventTest: EventTest? ->
                Assertions.assertEquals(0, result.get(), "child1 should be called before child2")
                result.set(1)
            }
        val child2 = EventNode.all("child2").setPriority(2)
            .addListener(EventTest::class.java) { eventTest: EventTest? ->
                Assertions.assertEquals(1, result.get(), "child2 should be called after child1")
                result.set(2)
            }
        node.addChild(child1)
        node.addChild(child2)
        Assertions.assertEquals(node.children.size, 2, "The node should have 2 children")
        node.call(EventTest())
        Assertions.assertEquals(2, result.get(), "The event should be called after the call")

        // Test removal
        result.set(0)
        node.removeChild(child2)
        Assertions.assertEquals(node.children.size, 1, "The node should have 1 child")
        node.call(EventTest())
        Assertions.assertEquals(1, result.get(), "child2 should has been removed")
        result.set(0)
        node.removeChild(child1)
        node.call(EventTest())
        Assertions.assertTrue(node.children.isEmpty(), "The node should have no child left")
        Assertions.assertEquals(0, result.get(), "The event should not be called after the removal")
    }
    ...
    fun testFiltering() {
        ...
        val node = EventNode.type(
            "item_node", EventFilter.ITEM
        ) { event: ItemEvent?, item: ItemStack -> item.material === Material.DIAMOND }
        val child = EventNode.type("item_node2", EventFilter.ITEM)
            .addListener(ItemTestEvent::class.java) { event: ItemTestEvent? -> childResult.set(true) }
        node.addChild(child)
        var listener = EventListener.of(
            ItemTestEvent::class.java
        ) { event: ItemTestEvent? -> Assertions.fail<Any>("The event should not be called") }
        node.addListener(listener)
        node.call(ItemTestEvent(ItemStack.of(Material.GOLD_BLOCK)))
        Assertions.assertFalse(childResult.get())
        node.removeListener(listener)
        listener = EventListener.of(ItemTestEvent::class.java) { event: ItemTestEvent? -> result.set(true) }
        node.addListener(listener)
        node.call(ItemTestEvent(ItemStack.of(Material.DIAMOND)))
        Assertions.assertTrue(result.get(), "The event should be called")
        Assertions.assertTrue(childResult.get(), "The child event should be called")
    }
    ...
    fun testBinding() {
        ...
        val result = AtomicBoolean(false)
        val binding =
            EventBinding.filtered(EventFilter.ITEM) { itemStack: ItemStack -> itemStack.material === Material.DIAMOND }
                .map(ItemTestEvent::class.java) { itemStack: ItemStack?, itemTestEvent: ItemTestEvent? ->
                    result.set(
                        true
                    )
                }
