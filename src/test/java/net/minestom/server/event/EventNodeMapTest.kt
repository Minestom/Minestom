...
class EventNodeMapTest {
    ...
    fun uniqueMapping() {
        ...
        val itemNode1 = node.map(item, EventFilter.ITEM)
        val itemNode2 = node.map(item, EventFilter.ITEM)
        Assertions.assertNotNull(itemNode1)
        Assertions.assertSame(itemNode1, itemNode2)
        ...
        node.unmap(item)
        Assertions.assertSame(itemNode1, node.map(item, EventFilter.ITEM))
    }

    @Test
    fun lazyRegistration() {
        val item = ItemStack.of(Material.DIAMOND)
        val node = EventNode.all("main") as EventNodeImpl<Event>
        val itemNode = node.map(item, EventFilter.ITEM)
        Assertions.assertFalse(node.registeredMappedNode.containsKey(item))
        itemNode.addListener(ItemTestEvent::class.java) { event: ItemTestEvent? -> }
        Assertions.assertTrue(node.registeredMappedNode.containsKey(item))
    }
    ...
    fun secondMap() {
        ...
        val node = EventNode.all("main") as EventNodeImpl<Event>
        val itemNode = node.map(item, EventFilter.ITEM)
        Assertions.assertSame(itemNode, itemNode.map(item, EventFilter.ITEM))
        Assertions.assertThrows(Exception::class.java) { itemNode.map(ItemStack.AIR, EventFilter.ITEM) }
    }
    ...
    fun map() {
        ...
        val itemNode = node.map(item, EventFilter.ITEM)
        Assertions.assertFalse(node.hasListener(ItemTestEvent::class.java))
        itemNode.addListener(ItemTestEvent::class.java) { event: ItemTestEvent? -> result.set(true) }
        Assertions.assertTrue(node.hasListener(ItemTestEvent::class.java))
        node.call(ItemTestEvent(item))
        Assertions.assertTrue(result.get())
        result.set(false)
        node.call(ItemTestEvent(ItemStack.of(Material.GOLD_INGOT)))
    ...
    fun entityLocal() {
        ...
        val node = process.eventHandler()
        val entity = Entity(EntityType.ZOMBIE)
        val result = AtomicBoolean(false)
        val listener = EventListener.of(
            EntityTestEvent::class.java
        ) { event: EntityTestEvent? -> result.set(true) }
        val handle = node.getHandle(
            EntityTestEvent::class.java
        )
        Assertions.assertFalse(handle.hasListener())
        entity.eventNode().addListener(listener)
        Assertions.assertTrue(handle.hasListener())
        Assertions.assertFalse(result.get())
        handle.call(EntityTestEvent(entity))
        Assertions.assertTrue(result.get())
        result.set(false)
    ...
    fun ownerGC() {
        ...
        var item: ItemStack? = ItemStack.of(Material.DIAMOND)
        val node = EventNode.all("main")
        val itemNode = node.map(item, EventFilter.ITEM)
        itemNode.addListener(ItemTestEvent::class.java) { event: ItemTestEvent? -> }
        node.call(ItemTestEvent(item))
