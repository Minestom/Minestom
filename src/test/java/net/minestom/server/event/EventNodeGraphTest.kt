...
class EventNodeGraphTest {
    ...
    fun single() {
        ...
        verifyGraph(node, EventNodeImpl.Graph("main", "Event", 0, List.of()))
    }
    ...
    fun singleChild() {
        ...
        verifyGraph(
            node, EventNodeImpl.Graph(
                "main", "Event", 0,
                List.of(
                    EventNodeImpl.Graph("child", "Event", 0, List.of())
                )
            )
        )
    }
        ...
        run {
            ...
            verifyGraph(
                node, EventNodeImpl.Graph(
                    "main", "Event", 0,
                    List.of(
                        EventNodeImpl.Graph("child1", "Event", 5, List.of()),
                        EventNodeImpl.Graph("child2", "Event", 10, List.of())
                    )
                )
            )
        }
    ...
    fun verifyGraph(n: EventNode<*>, graph: EventNodeImpl.Graph?) {
        val node = n as EventNodeImpl<*>
        val nodeGraph = node.createGraph()
        Assertions.assertEquals(graph, nodeGraph, "Graphs are not equals")
        Assertions.assertEquals(
            EventNodeImpl.createStringGraph(graph),
            EventNodeImpl.createStringGraph(nodeGraph),
            "String graphs are not equals"
        )
        Assertions.assertEquals(
            n.toString(),
            EventNodeImpl.createStringGraph(nodeGraph),
            "The node does not use createStringGraph"
        )
    }
}