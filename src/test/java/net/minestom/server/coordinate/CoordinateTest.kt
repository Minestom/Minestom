package net.minestom.server.coordinateimport
...
class CoordinateTest {
    ...
    fun chunkIndex() {
        ...
        Assertions.assertEquals(2, getChunkCoordX(index))
        Assertions.assertEquals(5, getChunkCoordZ(index))
        index = getChunkIndex(-5, 25)
        Assertions.assertEquals(-5, getChunkCoordX(index))
        Assertions.assertEquals(25, getChunkCoordZ(index))
        index = getChunkIndex(Int.MAX_VALUE, Int.MIN_VALUE)
        Assertions.assertEquals(Int.MAX_VALUE, getChunkCoordX(index))
        Assertions.assertEquals(Int.MIN_VALUE, getChunkCoordZ(index))
    }

    @Test
    fun chunkCoordinate() {
        Assertions.assertEquals(1, getChunkCoordinate(16))
        Assertions.assertEquals(-1, getChunkCoordinate(-16))
        Assertions.assertEquals(3, getChunkCoordinate(48))
        Assertions.assertEquals(4, getChunkCoordinate(65))
        Assertions.assertEquals(4, getChunkCoordinate(64))
        Assertions.assertEquals(3, getChunkCoordinate(63))
        Assertions.assertEquals(-2, getChunkCoordinate(-25))
        Assertions.assertEquals(23, getChunkCoordinate(380))
    }

    @Test
    fun chunkCount() {
        Assertions.assertEquals(289, getChunkCount(8))
        Assertions.assertEquals(169, getChunkCount(6))
        Assertions.assertEquals(121, getChunkCount(5))
        Assertions.assertEquals(9, getChunkCount(1))
        Assertions.assertEquals(1, getChunkCount(0))
        Assertions.assertThrows(IllegalArgumentException::class.java) { getChunkCount(-1) }
    }

    @Test
    fun vecAddition() {
        var temp = Vec.ZERO
        Assertions.assertEquals(0.0, temp.x())
        Assertions.assertEquals(0.0, temp.y())
        Assertions.assertEquals(0.0, temp.z())
        temp = temp.add(1.0)
        Assertions.assertEquals(1.0, temp.x())
        Assertions.assertEquals(1.0, temp.y())
        Assertions.assertEquals(1.0, temp.z())
        temp = temp.add(1.0, 0.0, 0.0)
        Assertions.assertEquals(2.0, temp.x())
        Assertions.assertEquals(1.0, temp.y())
        Assertions.assertEquals(1.0, temp.z())
        temp = temp.add(0.0, 1.0, 0.0)
        Assertions.assertEquals(2.0, temp.x())
        Assertions.assertEquals(2.0, temp.y())
        Assertions.assertEquals(1.0, temp.z())
        temp = temp.add(0.0, 0.0, 1.0)
        Assertions.assertEquals(2.0, temp.x())
        Assertions.assertEquals(2.0, temp.y())
        Assertions.assertEquals(2.0, temp.z())
    }

    @Test
    fun vecWith() {
        var temp = Vec.ZERO.withX(1.0)
        Assertions.assertEquals(1.0, temp.x())
        Assertions.assertEquals(0.0, temp.y())
        Assertions.assertEquals(0.0, temp.z())
        temp = temp.withX { x: Double -> x * 2 + 1 }
        Assertions.assertEquals(3.0, temp.x())
        Assertions.assertEquals(0.0, temp.y())
        Assertions.assertEquals(0.0, temp.z())
    }

    @Test
    fun toSectionRelativeCoordinate() {
        Assertions.assertEquals(8, toSectionRelativeCoordinate(-40))
        Assertions.assertEquals(12, toSectionRelativeCoordinate(-20))
        Assertions.assertEquals(0, toSectionRelativeCoordinate(0))
        Assertions.assertEquals(5, toSectionRelativeCoordinate(5))
        Assertions.assertEquals(15, toSectionRelativeCoordinate(15))
        Assertions.assertEquals(0, toSectionRelativeCoordinate(16))
        Assertions.assertEquals(4, toSectionRelativeCoordinate(20))
        Assertions.assertEquals(0, toSectionRelativeCoordinate(32))
        Assertions.assertEquals(1, toSectionRelativeCoordinate(33))
    }
}