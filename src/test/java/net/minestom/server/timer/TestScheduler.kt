...
class TestScheduler {
    ...
    fun tickTask() {
        ...
        Assertions.assertEquals(task.executionType(), ExecutionType.SYNC, "Tasks default execution type should be sync")
    ...
    fun durationTask() {
        ...
        Thread.sleep(100)
        scheduler.process()
        Assertions.assertFalse(result.get(), "900ms remaining")
        Thread.sleep(1200)
        scheduler.process()
        Assertions.assertTrue(result.get(), "Tick task must be executed after 1 second")
    }
    ...
    fun cancelTask() {
        ...
        val task = scheduler.buildTask { result.set(true) }
            .schedule()
        Assertions.assertTrue(task.isAlive, "Task should still be alive")
        task.cancel()
        Assertions.assertFalse(task.isAlive, "Task should not be alive anymore")
        scheduler.process()
        Assertions.assertFalse(result.get(), "Task should be cancelled")
    }
    ...
    fun parkTask() {
        val scheduler = Scheduler.newScheduler()
        // Ignored parked task
        scheduler.buildTask { Assertions.fail<Any>("This parked task should never be executed") }
        ...
        val task = scheduler.buildTask { result.set(true) }
            .delay(TaskSchedule.park())
            .schedule()
        Assertions.assertTrue(task.isParked)
        Assertions.assertFalse(result.get(), "Task hasn't been unparked yet")
        task.unpark()
        Assertions.assertFalse(task.isParked)
    ...
    fun asyncTask() {
        ...
        scheduler.buildTask {
            ...
            .schedule()
        Assertions.assertFalse(result.get(), "Async task should only be executed after process()")
        scheduler.process()
        Thread.sleep(250)
        Assertions.assertTrue(result.get(), "Async task didn't get executed")
    }
}