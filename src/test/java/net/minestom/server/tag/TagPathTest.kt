...
class TagPathTest {
    ...
    fun basic() {
        ...
        val path = tag.path("display")
        handler.setTag(path, 5)
        Assertions.assertEquals(5, handler.getTag(path))
        Assertions.assertNull(handler.getTag(tag))
        handler.setTag(path, 6)
        Assertions.assertEquals(6, handler.getTag(path))
        Assertions.assertNull(handler.getTag(tag))
        handler.removeTag(path)
        Assertions.assertNull(handler.getTag(path))
        Assertions.assertNull(handler.getTag(tag))
    }
    ...
    fun emptyRemoval() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("number").path("display")
    ...
    fun doubleSnbt() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("number").path("display")
        val tag1 = Tag.String("string").path("display")
        handler.setTag(tag, 5)
    ...
    fun overrideSnbt() {
        ...
        val tag1 = Tag.Integer("value").path("key")
        handler.setTag(tag, 5)
        TestUtils.assertEqualsSNBT(/* !!! Hit visitElement for element type: class org.jetbrains.kotlin.nj2k.tree.JKErrorExpression !!! */,
            handler.asCompound()
        )
        Assertions.assertThrows(IllegalStateException::class.java) { handler.setTag(tag1, 5) }
    }
    ...
    fun forgetPath() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("key")
        val path = Tag.Integer("value").path("key")
        handler.setTag(path, 5)
        Assertions.assertThrows(IllegalStateException::class.java) { handler.getTag(tag) }
    }
    ...
    fun chaining() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("key")
        val path = Tag.Integer("key").path("first", "second")
        handler.setTag(path, 5)
        TestUtils.assertEqualsSNBT(/* !!! Hit visitElement for element type: class org.jetbrains.kotlin.nj2k.tree.JKErrorExpression !!! */,
            handler.asCompound()
        )
    ...
    fun chainingDouble() {
        val handler = TagHandler.newHandler()
        val path = Tag.Integer("key").path("first", "second")
        val path1 = Tag.Integer("key").path("first")
        handler.setTag(path, 5)
        TestUtils.assertEqualsSNBT(/* !!! Hit visitElement for element type: class org.jetbrains.kotlin.nj2k.tree.JKErrorExpression !!! */,
            handler.asCompound()
        )
        ...
        Assertions.assertEquals(5, handler.getTag(path))
        Assertions.assertEquals(5, handler.getTag(path1))
        handler.removeTag(path)
        TestUtils.assertEqualsSNBT(/* !!! Hit visitElement for element type: class org.jetbrains.kotlin.nj2k.tree.JKErrorExpression !!! */,
            handler.asCompound()
        )
    ...
    @Test
    fun structureObstruction() {
        class Entry

        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("value")
        val struct = Tag.Structure("struct", object : TagSerializer<Entry?> {
            private static
            val VALUE_TAG = Tag.Integer("value")
            override fun read(reader: TagReadable): Entry? {
                val value = reader.getTag(VALUE_TAG)
                return value?.let { Entry(it) }
            }

            override fun write(writer: TagWritable, value: Entry?) {
                writer.setTag(VALUE_TAG, value?.value)
            }
        })
        handler.setTag(struct, Entry(5))
        TestUtils.assertEqualsSNBT(/* !!! Hit visitElement for element type: class org.jetbrains.kotlin.nj2k.tree.JKErrorExpression !!! */,
            handler.asCompound()
        )
        Assertions.assertEquals(5, handler.getTag(tag.path("struct")))
        handler.setTag(tag, 5)
        TestUtils.assertEqualsSNBT(/* !!! Hit visitElement for element type: class org.jetbrains.kotlin.nj2k.tree.JKErrorExpression !!! */,
            handler.asCompound()
        )
        ...
        Assertions.assertThrows(IllegalStateException::class.java) { handler.setTag(tag.path("struct"), 5) }
    }
    ...
    fun tagObstruction() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("key")
        val path = Tag.Integer("value").path("key", "second")
        handler.setTag(tag, 5)
        TestUtils.assertEqualsSNBT(/* !!! Hit visitElement for element type: class org.jetbrains.kotlin.nj2k.tree.JKErrorExpression !!! */,
            handler.asCompound()
        )
        Assertions.assertThrows(IllegalStateException::class.java) { handler.setTag(path, 5) }
    }
}