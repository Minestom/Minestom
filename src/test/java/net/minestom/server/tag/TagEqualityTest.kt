...
class TagEqualityTest {
    ...
    @Test
    fun same() {
        val tag = Tag.String("test")
        Assertions.assertTrue(tag.shareValue(tag))
    }

    @Test
    fun similar() {
        val tag = Tag.String("test")
        val tag2 = Tag.String("test")
        Assertions.assertTrue(tag.shareValue(tag2))
    }

    @Test
    fun differentDefault() {
        val tag = Tag.String("test").defaultValue("test2")
        val tag2 = Tag.String("test").defaultValue("test3")
        Assertions.assertTrue(tag.shareValue(tag2))
    }
    ...
    fun differentType() {
        val tag = Tag.String("test")
        val tag2 = Tag.Integer("test")
        Assertions.assertFalse(tag.shareValue(tag2))
    }

    @Test
    fun mapSame() {
        // Force identical functions
        val t1 = Function { value: Int? -> Entry(value) }
        val t2 = Function<Entry, Int> { obj: Entry -> obj.value() }
        val tag = Tag.Integer("key")
        val map1 = tag.map(t1, t2)
        val map2 = tag.map(t1, t2)
        Assertions.assertTrue(map1.shareValue(map2))
    }

    @Test
    fun mapChild() {
        val intTag = Tag.Integer("key")
        val tag = intTag.map(
            Function { value: Int? -> Entry(value) }, Function { obj: Entry -> obj.value() })
        Assertions.assertFalse(intTag.shareValue(tag))
    }

    @Test
    fun list() {
        val tag = Tag.String("test").list()
        Assertions.assertTrue(tag.shareValue(tag))
    }
    ...
    fun listScope() {
        val tag = Tag.String("test")
        Assertions.assertFalse(tag.shareValue(tag.list()))
    }

    @Test
    fun similarList() {
        val tag = Tag.String("test").list()
        val tag2 = Tag.String("test").list()
        Assertions.assertTrue(tag.shareValue(tag2))
        Assertions.assertTrue(tag.list().shareValue(tag2.list()))
    }
    ...
    fun differentList() {
        val tag = Tag.String("test").list()
        val tag2 = Tag.String("test").list()
        Assertions.assertFalse(tag.shareValue(tag2.list()))
        Assertions.assertFalse(tag.list().shareValue(tag2.list().list()))
    }

    @Test
    fun differentListType() {
        val tag = Tag.String("test").list()
        val tag2 = Tag.Integer("test").list()
        Assertions.assertFalse(tag.shareValue(tag2))
        Assertions.assertFalse(tag.list().shareValue(tag2.list()))
    }
}