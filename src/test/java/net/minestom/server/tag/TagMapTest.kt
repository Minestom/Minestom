...
class TagMapTest {
    ...
    fun map() {
        ...
        handler.setTag(tag, Entry(1))
        Assertions.assertEquals(1, handler.getTag(intTag))
        Assertions.assertEquals(Entry(1), handler.getTag(tag))
    }
    ...
    fun mapDefault() {
        ...
        val tag = intTag.map(
            Function { value: Int? -> Entry(value) }, Function { obj: Entry -> obj.value() })
        Assertions.assertEquals(Entry(1), handler.getTag<Entry>(tag.defaultValue(Entry(1))))
        handler.setTag(tag, Entry(2))
        Assertions.assertEquals(2, handler.getTag(intTag))
        Assertions.assertEquals(Entry(2), handler.getTag(tag))
    }
}