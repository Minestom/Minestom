...
class TagTest {
    @Test
    fun intGet() {
        val mutable = MutableNBTCompound().setInt("key", 5)
        val tag = Tag.Integer("key")
        val handler = TagHandler.fromCompound(MutableNBTCompound())
        handler.setTag(tag, 5)
        Assertions.assertEquals(5, handler.getTag(tag))
        Assertions.assertEquals(mutable.toCompound(), handler.asCompound(), "NBT is not the same")

        // Removal
        handler.setTag(tag, null)
        Assertions.assertEquals(NBTCompound(), handler.asCompound(), "Tag must be removed when set to null")
    }

    @Test
    fun intNull() {
        val handler = TagHandler.fromCompound(MutableNBTCompound().set("key", NBT.Int(5)))
        // Removal
        val tag = Tag.Integer("key")
        handler.setTag(tag, null)
        Assertions.assertFalse(handler.hasTag(tag))
        Assertions.assertEquals(NBTCompound.EMPTY, handler.asCompound(), "Tag must be removed when set to null")
    }
    ...
    fun snbt() {
        val mutable = MutableNBTCompound().setInt("key", 5)
        val reader = TagHandler.fromCompound(mutable)
        Assertions.assertEquals(reader.asCompound().toSNBT(), mutable.toCompound().toSNBT(), "SNBT is not the same")
    }

    @Test
    fun fromNbt() {
        val mutable = MutableNBTCompound().setInt("key", 5)
        val handler = TagHandler.fromCompound(mutable)
        Assertions.assertEquals(5, handler.getTag(Tag.Integer("key")))
        Assertions.assertEquals(mutable.toCompound(), handler.asCompound(), "NBT is not the same")
    }

    @Test
    fun defaultValue() {
        val nullable = Tag.String("key")
        val notNull = nullable.defaultValue("Hey")
        Assertions.assertNotSame(nullable, notNull)
        val handler = TagHandler.newHandler()
        Assertions.assertFalse(handler.hasTag(nullable))
        Assertions.assertTrue(handler.hasTag(notNull)) // default value is set
        Assertions.assertFalse(handler.hasTag(nullable))
        Assertions.assertNull(handler.getTag(nullable))
        Assertions.assertEquals("Hey", handler.getTag(notNull))
    }

    @Test
    fun invalidType() {
        val tag1 = Tag.Integer("key")
        val tag2 = Tag.String("key")
        val handler = TagHandler.newHandler()
        handler.setTag(tag1, 5)
        Assertions.assertEquals(5, handler.getTag(tag1))
        Assertions.assertNull(handler.getTag(tag2))
        Assertions.assertEquals("hey", handler.getTag(tag2.defaultValue("hey")))
    }

    @Test
    fun item() {
        val item = ItemStack.of(Material.DIAMOND)
        val tag = Tag.ItemStack("item")
        val handler = TagHandler.newHandler()
        handler.setTag(tag, item)
        Assertions.assertEquals(item, handler.getTag(tag))
    }

    @Test
    fun tagResizing() {
        val tag1 = Tag.Integer("tag1")
        val tag2 = Tag.Integer("tag2")
        val handler = TagHandler.newHandler()
        handler.setTag(tag1, 5)
        handler.setTag(tag2, 1)
        Assertions.assertEquals(5, handler.getTag(tag1))
        Assertions.assertEquals(1, handler.getTag(tag2))
    }

    @Test
    fun nbtResizing() {
        val handler = TagHandler.fromCompound(
            NBT.Compound(
                Map.of(
                    "tag1", NBT.Int(5),
                    "tag2", NBT.Int(1)
        ...
        Assertions.assertEquals(5, handler.getTag(Tag.Integer("tag1")))
        Assertions.assertEquals(1, handler.getTag(Tag.Integer("tag2")))
    }
}