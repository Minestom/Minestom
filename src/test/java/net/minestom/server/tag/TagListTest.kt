...
class TagListTest {
    ...
    fun basic() {
        ...
        val tag = Tag.Integer("number")
        val list = tag.list()
        handler.setTag(tag, 5)
        Assertions.assertEquals(5, handler.getTag(tag))
        Assertions.assertNull(handler.getTag(list))
        handler.setTag(list, List.of(1, 2, 3))
        Assertions.assertEquals(List.of(1, 2, 3), handler.getTag(list))
        Assertions.assertNull(handler.getTag(tag))
    }

    @Test
    fun cache() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("number").list()
        val `val` = List.of(1, 2, 3)
        handler.setTag(tag, `val`)
        Assertions.assertSame(`val`, handler.getTag(tag))
    }

    @Test
    fun recursiveCache() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("number").list().list()
        val `val` = List.of(List.of(1, 2, 3), List.of(4, 5, 6))
        handler.setTag(tag, `val`)
        Assertions.assertSame(`val`[0], handler.getTag(tag)[0])
        Assertions.assertSame(`val`[1], handler.getTag(tag)[1])
        Assertions.assertSame(`val`, handler.getTag(tag))
    }

    @Test
    fun recursiveCacheIncorrect() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("number").list().list()
        val `val` = List.of(List.of(1, 2, 3), ArrayList(Arrays.asList(4, 5, 6)))
        handler.setTag(tag, `val`)
        Assertions.assertSame(`val`[0], handler.getTag(tag)[0])
        Assertions.assertNotSame(`val`[1], handler.getTag(tag)[1])
        Assertions.assertNotSame(`val`, handler.getTag(tag))
        Assertions.assertEquals(`val`, handler.getTag(tag))
    }

    @Test
    fun snbt() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("numbers").list()
        handler.setTag(tag, List.of(1, 2, 3))
        TestUtils.assertEqualsSNBT(/* !!! Hit visitElement for element type: class org.jetbrains.kotlin.nj2k.tree.JKErrorExpression !!! */,
            handler.asCompound()
        )
    }
    ...
    fun empty() {
        ...
        Assertions.assertEquals(List.of<Any>(), handler.getTag(tag))
    }
    ...
    fun removal() {
        ...
        Assertions.assertEquals(List.of(1), handler.getTag(tag))
    ...
    fun removalSnbt() {
        ...
        handler.removeTag(tag)
        TestUtils.assertEqualsSNBT("{}", handler.asCompound())
    }

    @Test
    fun chaining() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("numbers").list().list()
        val integers = List.of(List.of(1, 2, 3), List.of(4, 5, 6))
        handler.setTag(tag, integers)
        Assertions.assertEquals(integers, handler.getTag(tag))
        handler.removeTag(tag)
        Assertions.assertNull(handler.getTag(tag))
    }
    ...
    fun defaultValue() {
        val handler = TagHandler.newHandler()
        val `val` = List.of(1, 2, 3)
        val tag = Tag.Integer("number").list().defaultValue(`val`)
        Assertions.assertEquals(List.of(1, 2, 3), handler.getTag(tag))
    }

    @Test
    fun defaultValueReset() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("number").defaultValue(5)
        val list = tag.list()
        Assertions.assertNull(handler.getTag(list))
        Assertions.assertEquals(List.of(1, 2, 3), handler.getTag(list.defaultValue(List.of(1, 2, 3))))
    }
    ...
    fun immutability() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("number").list()
        val `val`: MutableList<Int> = ArrayList()
        `val`.add(1)
        handler.setTag(tag, `val`)
        Assertions.assertNotSame(`val`, handler.getTag(tag))
        Assertions.assertEquals(List.of(1), handler.getTag(tag))
    ...
    @Test
    fun chainingImmutability() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("numbers").list().list()
        val `val`: MutableList<MutableList<Int>> = ArrayList()
        `val`.add(ArrayList(Arrays.asList(1, 2, 3)))
        `val`.add(ArrayList(Arrays.asList(4, 5, 6)))
        handler.setTag(tag, `val`)
        Assertions.assertNotSame(`val`, handler.getTag(tag))
        Assertions.assertEquals(List.of(List.of(1, 2, 3), List.of(4, 5, 6)), handler.getTag(tag))

        // Must not modify the nbt
        `val`[0].add(7)
        `val`[1].add(8)
        `val`.add(ArrayList(Arrays.asList(9, 10, 11)))
        Assertions.assertNotSame(`val`, handler.getTag(tag))
        Assertions.assertEquals(List.of(List.of(1, 2, 3), List.of(4, 5, 6)), handler.getTag(tag))
    }

    @Test
    fun immutabilitySnbt() {
        val handler = TagHandler.newHandler()
        val tag = Tag.Integer("numbers").list()
        val `val`: MutableList<Int> = ArrayList()
        `val`.add(1)
