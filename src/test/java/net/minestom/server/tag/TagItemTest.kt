...
class TagItemTest {
    ...
    fun get() {
        ...
        Assertions.assertSame(item, handler.getTag(tag))
    }

    @get:Test
    val differentObject: Unit
        get() {
            val item = ItemStack.of(Material.DIAMOND)
            val handler = TagHandler.newHandler()
            handler.setTag(Tag.ItemStack("item"), item)
            Assertions.assertEquals(item, handler.getTag(Tag.ItemStack("item")))
        }
    ...
    fun invalidation() {
        val item = ItemStack.of(Material.DIAMOND)
        val item2 = ItemStack.of(Material.DIAMOND, 2)
        val handler = TagHandler.newHandler()
        val tag = Tag.ItemStack("item")
        handler.setTag(tag, item)
        Assertions.assertSame(item, handler.getTag(tag))
        handler.setTag(tag, item2)
        Assertions.assertSame(item2, handler.getTag(tag))
    }

    @Test
    fun differentTagInvalidation() {
        val item = ItemStack.of(Material.DIAMOND)
        val item2 = ItemStack.of(Material.DIAMOND, 2)
        val handler = TagHandler.newHandler()
        val itemTag = Tag.ItemStack("item")
        val nbtTag = Tag.NBT<NBT>("item")
        // Write the item using the ItemStack tag
        run {
            handler.setTag(itemTag, item)
            Assertions.assertSame(item, handler.getTag(itemTag))
            Assertions.assertEquals(item.toItemNBT(), handler.getTag(nbtTag))
        }
        // Override it with an NBT tag
        run {
            handler.setTag(nbtTag, item2.toItemNBT())
            Assertions.assertEquals(item2, handler.getTag(itemTag))
            Assertions.assertEquals(item2.toItemNBT(), handler.getTag(nbtTag))
        }
    }
}