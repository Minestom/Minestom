...
class TagStructureTest {
    ...
    fun basic() {
        ...
        Assertions.assertNull(handler.getTag(STRUCTURE_TAG))
        Assertions.assertFalse(handler.hasTag(STRUCTURE_TAG))
        val entry: Entry = Entry("hello")
        handler.setTag(STRUCTURE_TAG, entry)
        Assertions.assertTrue(handler.hasTag(STRUCTURE_TAG))
        Assertions.assertEquals(entry, handler.getTag(STRUCTURE_TAG))
    ...
    fun overrideBasic() {
        ...
        Assertions.assertNull(handler.getTag(STRUCTURE_TAG))
        Assertions.assertFalse(handler.hasTag(STRUCTURE_TAG))
        val entry1: Entry = Entry("hello")
        val entry2: Entry = Entry("hello2")

        // Add first entry
        run {
            handler.setTag(STRUCTURE_TAG, entry1)
            Assertions.assertTrue(handler.hasTag(STRUCTURE_TAG))
            Assertions.assertEquals(entry1, handler.getTag(STRUCTURE_TAG))
        }
        // Add second entry
        run {
            handler.setTag(STRUCTURE_TAG2, entry2)
            Assertions.assertTrue(handler.hasTag(STRUCTURE_TAG2))
            Assertions.assertEquals(entry2, handler.getTag(STRUCTURE_TAG2))
            // Assert first
            Assertions.assertFalse(handler.hasTag(STRUCTURE_TAG))
            Assertions.assertNull(handler.getTag(STRUCTURE_TAG))
        }
    ...
    companion object {
        private val STRUCTURE_TAG = Tag.Structure("entry", object : TagSerializer<Entry?> {
            private static
            val VALUE_TAG = Tag.String("value")
            override fun read(reader: TagReadable): Entry? {
                val value = reader.getTag(VALUE_TAG)
                return if (value != null) Entry(value) else null
            }
        ...
        private val STRUCTURE_TAG2 = Tag.Structure("entry", object : TagSerializer<Entry?> {
            private static
            val VALUE_TAG = Tag.String("value2")
            override fun read(reader: TagReadable): Entry? {
                val value = reader.getTag(VALUE_TAG)
                return if (value != null) Entry(value) else null
            }
