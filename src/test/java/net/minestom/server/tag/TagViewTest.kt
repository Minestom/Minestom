...
class TagViewTest {
    ...
    fun basic() {
        ...
        Assertions.assertNull(handler.getTag(VIEW_TAG))
        Assertions.assertFalse(handler.hasTag(VIEW_TAG))
        val entry: Entry = Entry("hello")
        handler.setTag(VIEW_TAG, entry)
        Assertions.assertTrue(handler.hasTag(VIEW_TAG))
        Assertions.assertEquals(entry, handler.getTag(VIEW_TAG))
    ...
    companion object {
        private val VIEW_TAG = Tag.View(object : TagSerializer<Entry?> {
            private static
