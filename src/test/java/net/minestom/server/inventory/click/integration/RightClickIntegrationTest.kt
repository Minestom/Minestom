package net.minestom.server.inventory.click.integrationimport
...
class RightClickIntegrationTest {
    @Test
    fun rightSelf(env: Env) {
        val instance = env.createFlatInstance()
        val player = env.createPlayer(instance, Pos(0, 40, 0))
        val inventory = player.inventory
        val listener: FlexibleListener<InventoryPreClickEvent?> = env.listen(
        ...
        // Empty click
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                Assertions.assertNull(event!!.inventory) // Player inventory
                Assertions.assertEquals(0, event.slot)
                Assertions.assertEquals(ClickType.RIGHT_CLICK, event.clickType)
                Assertions.assertEquals(ItemStack.AIR, inventory.cursorItem)
            }
            rightClick(player, 0)
        }
        // Pickup diamond
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                Assertions.assertEquals(1, event!!.slot)
                Assertions.assertEquals(ItemStack.AIR, inventory.cursorItem)
                Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.getItemStack(1))
            }
            rightClick(player, 1)
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.cursorItem)
            Assertions.assertEquals(ItemStack.AIR, inventory.getItemStack(1))
        }
        // Place it back
        run {
            listener.followup { event: InventoryPreClickEvent? ->
        ...
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                Assertions.assertEquals(2, event!!.slot)
                Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.cursorItem)
                Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.getItemStack(2))
            }
            rightClick(player, 2)
            Assertions.assertEquals(ItemStack.AIR, inventory.cursorItem)
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND, 2), inventory.getItemStack(2))
        }
        // Cancel event
        run {
            listener.followup { event: InventoryPreClickEvent? -> event!!.isCancelled = true }
            rightClick(player, 2)
            Assertions.assertEquals(ItemStack.AIR, inventory.cursorItem, "Left click cancellation did not work")
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND, 2), inventory.getItemStack(2))
        }
        // Change items
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                event!!.clickedItem = ItemStack.of(Material.DIAMOND, 5)
                event.cursorItem = ItemStack.of(Material.DIAMOND)
            }
            rightClick(player, 1)
            Assertions.assertEquals(ItemStack.AIR, inventory.cursorItem)
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND, 6), inventory.getItemStack(1))
        }
    }

    @Test
    fun rightExternal(env: Env) {
        val instance = env.createFlatInstance()
        val player = env.createPlayer(instance, Pos(0, 40, 0))
        val inventory = Inventory(InventoryType.HOPPER, "test")
        ...
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                ...
                Assertions.assertEquals(0, event.slot)
                Assertions.assertEquals(ClickType.RIGHT_CLICK, event.clickType)
                Assertions.assertEquals(ItemStack.AIR, inventory.getCursorItem(player))
            }
            rightClick(player, 0)
        }
        // Pickup diamond
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                Assertions.assertEquals(inventory, event!!.inventory)
                Assertions.assertEquals(1, event.slot)
                Assertions.assertEquals(ItemStack.AIR, inventory.getCursorItem(player))
                Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.getItemStack(1))
            }
            rightClickOpenInventory(player, 1)
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.getCursorItem(player))
            Assertions.assertEquals(ItemStack.AIR, inventory.getItemStack(1))
        }
        // Place back to player inv
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                Assertions.assertNull(event!!.inventory)
                Assertions.assertEquals(1, event.slot)
                Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.getCursorItem(player))
                Assertions.assertEquals(ItemStack.AIR, inventory.getItemStack(1))
                Assertions.assertEquals(ItemStack.AIR, player.inventory.getItemStack(1))
            }
            rightClick(player, 1)
            Assertions.assertEquals(ItemStack.AIR, inventory.getCursorItem(player))
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND), player.inventory.getItemStack(1))
        }
        // Cancel event
        run {
            listener.followup { event: InventoryPreClickEvent? -> event!!.isCancelled = true }
            ...
            Assertions.assertEquals(
                ...
                player.inventory.getItemStack(1),
                "Left click cancellation did not work"
            )
            Assertions.assertEquals(ItemStack.AIR, inventory.getCursorItem(player))
        }
        // Change items
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                Assertions.assertNull(event!!.inventory)
                Assertions.assertEquals(9, event.slot)
                event.clickedItem = ItemStack.of(Material.DIAMOND, 5)
                event.cursorItem = ItemStack.of(Material.DIAMOND)
            }
            rightClick(player, 9)
            Assertions.assertEquals(ItemStack.AIR, inventory.getCursorItem(player))
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND, 6), player.inventory.getItemStack(9))
        }
    }

    private fun rightClickOpenInventory(player: Player, slot: Int) {
        _rightClick(player.openInventory, true, player, slot)
    }

    private fun rightClick(player: Player, slot: Int) {
        _rightClick(player.openInventory, false, player, slot)
    }

    private fun _rightClick(openInventory: Inventory?, clickOpenInventory: Boolean, player: Player, slot: Int) {
        var slot = slot
        val windowId = openInventory?.windowId ?: 0
        if (clickOpenInventory) {
            assert(openInventory != null)
            // Do not touch slot
        } else {
            val offset = openInventory?.innerSize ?: 0
            slot = convertToPacketSlot(slot)
        ...
        player.addPacketToQueue(
            ClientClickWindowPacket(
                windowId, 0, slot.toShort(), 1.toByte(),
