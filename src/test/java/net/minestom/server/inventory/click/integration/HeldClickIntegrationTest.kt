...
class HeldClickIntegrationTest {
    ...
    fun heldSelf(env: Env) {
        ...
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                ...
                Assertions.assertEquals(3, event.slot)
                Assertions.assertEquals(ClickType.CHANGE_HELD, event.clickType)
                Assertions.assertEquals(ItemStack.AIR, inventory.cursorItem)
                Assertions.assertEquals(ItemStack.AIR, event.cursorItem)
                Assertions.assertEquals(ItemStack.AIR, event.clickedItem)
            }
        ...
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                ...
                Assertions.assertEquals(ItemStack.AIR, inventory.cursorItem)
                Assertions.assertEquals(ItemStack.AIR, event.cursorItem)
                Assertions.assertEquals(ItemStack.of(Material.DIAMOND), event.clickedItem)
            }
            heldClick(player, 1, 0)
            Assertions.assertEquals(ItemStack.AIR, inventory.cursorItem)
            Assertions.assertEquals(ItemStack.AIR, inventory.getItemStack(1))
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.getItemStack(0))
        }
        // Swap items
        run {
            listener.followup { event: InventoryPreClickEvent? ->
            ...
            Assertions.assertEquals(ItemStack.of(Material.GOLD_INGOT), inventory.getItemStack(0))
        }
        // Cancel event
        run {
    ...
    fun heldExternal(env: Env) {
        ...
        run {
            ...
            Assertions.assertEquals(ItemStack.AIR, inventory.getCursorItem(player))
            Assertions.assertEquals(ItemStack.AIR, inventory.getItemStack(0))
        }
        // Swap empty
        run {
            listener.followup { event: InventoryPreClickEvent? ->
            ...
            Assertions.assertEquals(ItemStack.AIR, inventory.getCursorItem(player))
            Assertions.assertEquals(ItemStack.AIR, inventory.getItemStack(1))
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND), playerInv.getItemStack(0))
        }
        // Swap items
        run {
            listener.followup { event: InventoryPreClickEvent? ->
            ...
            Assertions.assertEquals(ItemStack.AIR, inventory.getCursorItem(player))
            Assertions.assertEquals(ItemStack.of(Material.DIAMOND), inventory.getItemStack(2))
            Assertions.assertEquals(ItemStack.of(Material.GOLD_INGOT), playerInv.getItemStack(0))
        }
        // Cancel event
        run {
            listener.followup { event: InventoryPreClickEvent? -> event!!.isCancelled = true }
    ...
    private fun heldClickOpenInventory(player: Player, slot: Int, target: Int) {
        _heldClick(player.openInventory, true, player, slot, target)
    }

    private fun heldClick(player: Player, slot: Int, target: Int) {
        _heldClick(player.openInventory, false, player, slot, target)
    }

    private fun _heldClick(
        openInventory: Inventory?,
        clickOpenInventory: Boolean,
        player: Player,
        slot: Int,
        target: Int
    ) {
        ...
        player.addPacketToQueue(
            ClientClickWindowPacket(
                windowId, 0, slot.toShort(), target.toByte(),
