...
class LeftClickIntegrationTest {
    ...
    fun leftSelf(env: Env) {
        ...
        run {
            listener.followup { event: InventoryPreClickEvent? ->
                ...
                Assertions.assertEquals(ClickType.LEFT_CLICK, event.clickType)
    ...
    fun leftExternal(env: Env) {
        ...
        run {
            ...
            Assertions.assertEquals(
                ItemStack.AIR,
                inventory.getCursorItem(player),
    ...
    private fun leftClickOpenInventory(player: Player, slot: Int) {
        _leftClick(player.openInventory, true, player, slot)
    }

    private fun leftClick(player: Player, slot: Int) {
        _leftClick(player.openInventory, false, player, slot)
    }

    private fun _leftClick(openInventory: Inventory?, clickOpenInventory: Boolean, player: Player, slot: Int) {
        var slot = slot
        ...
        player.addPacketToQueue(
            ClientClickWindowPacket(
                windowId, 0, slot.toShort(), 0.toByte(),
