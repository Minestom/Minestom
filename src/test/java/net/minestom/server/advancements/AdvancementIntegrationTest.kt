package net.minestom.server.advancementsimport
...
class AdvancementIntegrationTest {
    @Test
    fun addAndRemoveViewer(env: Env) {
        ...
        val tab = env.process().advancement().createTab("minestom:minestom_tab", root)

        // Add viewer
        tab.addViewer(player)
        Assertions.assertEquals(1, tab.viewers.size)
        Assertions.assertTrue(tab.viewers.contains(player))
        Assertions.assertNotNull(getTabs(player))
        Assertions.assertEquals(1, getTabs(player)!!.size)
        Assertions.assertTrue(getTabs(player)!!.contains(tab))

        // Remove viewer
        tab.removeViewer(player)
        Assertions.assertEquals(0, tab.viewers.size)
        Assertions.assertNull(getTabs(player))
    }

    @Test
    fun removeViewerOnDisconnect(env: Env) {
        val instance = env.createFlatInstance()
        val player = env.createPlayer(instance, Pos(0, 42, 0))
        val root1 = AdvancementRoot(
            Component.text("title"),
            Component.text("description"),
            Material.DIAMOND,
            FrameType.TASK,
        ...
        val tab1 = env.process().advancement().createTab("minestom:minestom_tab1", root1)
        val tab2 = env.process().advancement().createTab("minestom:minestom_tab2", root2)
        tab1.addViewer(player)
        tab2.addViewer(player)
        player.remove() // Disconnect
        Assertions.assertEquals(0, tab1.viewers.size)
        Assertions.assertEquals(0, tab2.viewers.size)
        Assertions.assertNull(getTabs(player))
    }
}