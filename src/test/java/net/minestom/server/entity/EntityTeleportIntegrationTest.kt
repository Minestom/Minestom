...
class EntityTeleportIntegrationTest {
    @Test
    fun entityChunkTeleport(env: Env) {
        val instance = env.createFlatInstance()
        val entity = Entity(EntityTypes.ZOMBIE)
        entity.setInstance(instance, Pos(0, 42, 0)).join()
        Assertions.assertEquals(instance, entity.getInstance())
        Assertions.assertEquals(Pos(0, 42, 0), entity.getPosition())
        entity.teleport(Pos(1, 42, 1)).join()
        Assertions.assertEquals(Pos(1, 42, 1), entity.getPosition())
    }

    @Test
    fun entityTeleport(env: Env) {
        val instance = env.createFlatInstance()
        val entity = Entity(EntityTypes.ZOMBIE)
        entity.setInstance(instance, Pos(0, 42, 0)).join()
        Assertions.assertEquals(instance, entity.getInstance())
        Assertions.assertEquals(Pos(0, 42, 0), entity.getPosition())
        entity.teleport(Pos(52, 42, 52)).join()
        Assertions.assertEquals(Pos(52, 42, 52), entity.getPosition())
    }

    @Test
    fun playerChunkTeleport(env: Env) {
        val instance = env.createFlatInstance()
        val connection = env.createConnection()
        val player = connection.connect(instance, Pos(0, 40, 0)).join()
        Assertions.assertEquals(instance, player.getInstance())
        Assertions.assertEquals(Pos(0, 40, 0), player.getPosition())
        val viewerConnection = env.createConnection()
        viewerConnection.connect(instance, Pos(0, 40, 0)).join()
        val tracker = connection.trackIncoming(
            ServerPacket::class.java
        )
        val viewerTracker = viewerConnection.trackIncoming(
            ServerPacket::class.java
        )
        val teleportPosition: Pos = Pos(1, 42, 1)
        player.teleport(teleportPosition).join()
        Assertions.assertEquals(teleportPosition, player.getPosition())

        // Verify received packet(s)
        tracker.assertSingle(
            PlayerPositionAndLookPacket::class.java,
            Consumer { packet: PlayerPositionAndLookPacket? ->
        ...
        viewerTracker.assertSingle(EntityTeleportPacket::class.java, Consumer { packet: EntityTeleportPacket? ->
            Assertions.assertEquals(player.entityId, packet!!.entityId())
            Assertions.assertEquals(teleportPosition, packet.position())
        })
    }

    @Test
    fun playerTeleport(env: Env) {
        val instance = env.createFlatInstance()
        val connection = env.createConnection()
        val player = connection.connect(instance, Pos(0, 40, 0)).join()
        Assertions.assertEquals(instance, player.getInstance())
        ...
        val viewerConnection = env.createConnection()
        viewerConnection.connect(instance, Pos(0, 40, 0)).join()
        val teleportPosition: Pos = Pos(4999, 42, 4999)
        player.teleport(teleportPosition).join()
        Assertions.assertEquals(teleportPosition, player.getPosition())
    }
}