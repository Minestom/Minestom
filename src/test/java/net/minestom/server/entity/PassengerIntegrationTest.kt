...
class PassengerIntegrationTest {
    ...
    fun passenger(env: Env) {
        ...
        passenger.setInstance(instance, Pos(0, 40, 0)).join()
        Assertions.assertEquals(0, vehicle.passengers.size)
        Assertions.assertNull(passenger.getVehicle())
        vehicle.addPassenger(passenger)
        Assertions.assertEquals(1, vehicle.passengers.size)
        Assertions.assertEquals(vehicle, passenger.getVehicle())
    }

    @Test
    fun passengerTeleport(env: Env) {
        val instance = env.createFlatInstance()
        val vehicle = Entity(EntityType.ZOMBIE)
        val passenger = Entity(EntityType.ZOMBIE)
        vehicle.setInstance(instance, Pos(0, 40, 0)).join()
        passenger.setInstance(instance, Pos(0, 40, 5000)).join()
        Assertions.assertEquals(0, vehicle.passengers.size)
        Assertions.assertNull(passenger.getVehicle())
        vehicle.addPassenger(passenger)
        Assertions.assertEquals(1, vehicle.passengers.size)
        Assertions.assertEquals(vehicle, passenger.getVehicle())
        Assertions.assertTrue(passenger.getDistance(vehicle) < 2)
    }
}