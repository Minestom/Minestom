...
class PlayerHeldIntegrationTest {
    ...
    fun playerHeld(env: Env) {
        ...
        Assertions.assertEquals(ItemStack.AIR, player.itemInMainHand)
        Assertions.assertEquals(0, player.heldSlot.toInt())
        player.addPacketToQueue(ClientHeldItemChangePacket(1.toShort()))
        player.interpretPacketQueue()
        Assertions.assertEquals(ItemStack.of(Material.STONE), player.itemInMainHand)
        Assertions.assertEquals(1, player.heldSlot.toInt())
    }

    @Test
    fun playerHeldEvent(env: Env) {
        val instance = env.createFlatInstance()
        val connection = env.createConnection()
        val player = connection.connect(instance, Pos(0, 40, 0)).join()
        player.getInventory().setItemStack(1, ItemStack.of(Material.STONE))
        ...
        Assertions.assertEquals(0, player.heldSlot.toInt())
        player.addPacketToQueue(ClientHeldItemChangePacket(1.toShort()))
        val listener: FlexibleListener<PlayerChangeHeldSlotEvent?> = env.listen(
            PlayerChangeHeldSlotEvent::class.java
        )
        listener.followup { event: PlayerChangeHeldSlotEvent? ->
            Assertions.assertEquals(player, event!!.player)
            Assertions.assertEquals(1, event.slot.toInt())
        }
        player.interpretPacketQueue()
        Assertions.assertEquals(ItemStack.of(Material.STONE), player.itemInMainHand)
        Assertions.assertEquals(1, player.heldSlot.toInt())
