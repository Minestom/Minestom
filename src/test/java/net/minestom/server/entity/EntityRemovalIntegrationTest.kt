...
class EntityRemovalIntegrationTest {
    ...
    fun destructionPacket(env: Env) {
        ...
        entity.setInstance(instance, Pos(0, 40, 0)).join()
        val tracker = connection.trackIncoming(
            DestroyEntitiesPacket::class.java
        )
        entity.remove()
        tracker.assertSingle { packet: DestroyEntitiesPacket? ->
            Assertions.assertEquals(
                List.of(entity.entityId),
                packet!!.entityIds()
            )
        }
    }
    ...
    @Test
    fun instanceRemoval(env: Env) {
        val instance = env.createFlatInstance()
        val entity = Entity(EntityType.ZOMBIE)
        entity.setInstance(instance, Pos(0, 40, 0)).join()
        Assertions.assertFalse(entity.isRemoved)
        entity.remove()
        Assertions.assertTrue(entity.isRemoved)
        Assertions.assertFalse(instance.entities.contains(entity), "Entity must not be in the instance anymore")
    }

    @Test
    @Throws(InterruptedException::class)
    fun tickTimedRemoval(env: Env) {
        val instance = env.createFlatInstance()
        val entity = TestEntity(2, TimeUnit.SERVER_TICK)
        entity.setInstance(instance, Pos(0, 40, 0)).join()
        Assertions.assertFalse(entity.isRemoved)
        Assertions.assertEquals(0, entity.aliveTicks)
        Thread.sleep(150) // Ensure that time is not used for tick scheduling
        env.tick()
        Assertions.assertFalse(entity.isRemoved)
        Assertions.assertEquals(1, entity.aliveTicks)
        env.tick()
        Assertions.assertTrue(entity.isRemoved)
        Assertions.assertEquals(1, entity.aliveTicks)
    }
    ...
    @Test
    fun entityGC(env: Env) {
        // Ensure that entities do not stay in memory after they are removed
        val instance = env.createFlatInstance()
        var entity: Entity? = Entity(EntityType.ZOMBIE)
        entity!!.setInstance(instance, Pos(0, 40, 0)).join()
        entity.remove()
        val ref = WeakReference(entity)
        entity = null
    ...
    @Test
    fun entityNodeGC(env: Env) {
        // Ensure that the entities GCed when a local listener is present
        val node = env.process().eventHandler()
        var entity: Entity? = Entity(EntityType.ZOMBIE)
        entity!!.eventNode().addListener(EntityTickEvent::class.java) { event: EntityTickEvent? -> }
        node.call(EntityTickEvent(entity))
        val ref = WeakReference(entity)
        entity.remove()
        entity = null
        env.tick()
    ...
    internal class TestEntity(delay: Long, unit: TemporalUnit?) : Entity(EntityType.ZOMBIE) {
        init {
