...
class EntityViewIntegrationTest {
    @Test
    fun emptyEntity(env: Env) {
        val instance = env.createFlatInstance()
        val entity = Entity(EntityType.ZOMBIE)
        entity.setInstance(instance, Pos(0, 40, 42)).join()
        Assertions.assertEquals(0, entity.getViewers().size)
    }

    @Test
    fun emptyPlayer(env: Env) {
        val instance = env.createFlatInstance()
        val player = env.createPlayer(instance, Pos(0, 42, 0))
        Assertions.assertEquals(0, player.getViewers().size)
    }

    @Test
    fun multiPlayers(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 42))
        val p2 = env.createPlayer(instance, Pos(0, 42, 42))
        Assertions.assertEquals(1, p1.getViewers().size)
        p1.getViewers().forEach(Consumer { p: Player? -> Assertions.assertEquals(p2, p) })
        Assertions.assertEquals(1, p2.getViewers().size)
        p2.getViewers().forEach(Consumer { p: Player? -> Assertions.assertEquals(p1, p) })
        p2.remove()
        Assertions.assertEquals(0, p1.getViewers().size)
        Assertions.assertEquals(0, p2.getViewers().size)
        val p3 = env.createPlayer(instance, Pos(0, 42, 42))
        Assertions.assertEquals(1, p1.getViewers().size)
        p1.getViewers().forEach(Consumer { p: Player? -> Assertions.assertEquals(p3, p) })
    }

    @Test
    fun manualViewers(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 0))
        val p2 = env.createPlayer(instance, Pos(0, 42, 5000))
        Assertions.assertEquals(0, p1.getViewers().size)
        Assertions.assertEquals(0, p2.getViewers().size)
        p1.addViewer(p2)
        Assertions.assertEquals(1, p1.getViewers().size)
        Assertions.assertEquals(0, p2.getViewers().size)
        p2.teleport(Pos(0, 42, 0)).join()
        Assertions.assertEquals(1, p1.getViewers().size)
        Assertions.assertEquals(1, p2.getViewers().size)
    }

    @Test
    fun movements(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 0))
        val p2 = env.createPlayer(instance, Pos(0, 42, 96))
        Assertions.assertEquals(0, p1.getViewers().size)
        Assertions.assertEquals(0, p2.getViewers().size)
        p2.teleport(Pos(0, 42, 95)).join() // Teleport in range (6 chunks)
        Assertions.assertEquals(1, p1.getViewers().size)
        Assertions.assertEquals(1, p2.getViewers().size)
    }

    @Test
    fun autoViewable(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 0))
        Assertions.assertTrue(p1.isAutoViewable)
        p1.isAutoViewable = false
        val p2 = env.createPlayer(instance, Pos(0, 42, 0))
        Assertions.assertEquals(0, p1.getViewers().size)
        Assertions.assertEquals(1, p2.getViewers().size)
        p1.isAutoViewable = true
        Assertions.assertEquals(1, p1.getViewers().size)
        Assertions.assertEquals(1, p2.getViewers().size)
    }

    @Test
    fun predictableViewers(env: Env) {
        val instance = env.createFlatInstance()
        val p = env.createPlayer(instance, Pos(0, 42, 0))
        Assertions.assertTrue(p.hasPredictableViewers())
        p.isAutoViewable = false
        Assertions.assertFalse(p.hasPredictableViewers())
        p.isAutoViewable = true
        Assertions.assertTrue(p.hasPredictableViewers())
        // MANUAL VIEWERS
        run {
            val tmpPlayer = env.createPlayer(instance, Pos(0, 42, 0))
        ...
        // CHANGE RULE
        run {
            p.updateViewableRule { player: Player? -> false }
            Assertions.assertFalse(p.hasPredictableViewers())
    ...
    @Test
    fun livingVehicle(env: Env) {
        val instance = env.createFlatInstance()
        val connection = env.createConnection()
        val player = connection.connect(instance, Pos(0, 40, 0)).join()
        val vehicle = Entity(EntityType.ZOMBIE)
        val passenger = Entity(EntityType.ZOMBIE)
        val tracker = connection.trackIncoming(
            SpawnLivingEntityPacket::class.java
        )
        vehicle.setInstance(instance, Pos(0, 40, 0)).join()
        vehicle.addPassenger(passenger)
        // Verify packets
        run {
            val results: List<SpawnLivingEntityPacket?> = tracker.collect()
            Assertions.assertEquals(2, results.size)
            Assertions.assertEquals(vehicle.entityId, results[0]!!.entityId())
            Assertions.assertEquals(passenger.entityId, results[1]!!.entityId())
        }
        // Verify viewers
        run {
            Assertions.assertEquals(0, player.getViewers().size)
            Assertions.assertEquals(1, vehicle.getViewers().size)
            Assertions.assertTrue(vehicle.isViewer(player))
            Assertions.assertEquals(1, passenger.getViewers().size)
            Assertions.assertTrue(passenger.isViewer(player))
        }
    }

    @Test
    fun vehicleInheritance(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 40, 0))
        val p2 = env.createPlayer(instance, Pos(0, 40, 0))
        val vehicle = Entity(EntityType.ZOMBIE)
        vehicle.setInstance(instance, Pos(0, 40, 0)).join()
        vehicle.addPassenger(p1)
        val vehicle2 = Entity(EntityType.ZOMBIE)
        vehicle2.setInstance(instance, Pos(0, 40, 0)).join()
        vehicle2.addPassenger(p2)
        Assertions.assertEquals(2, vehicle.getViewers().size)
        Assertions.assertEquals(2, vehicle2.getViewers().size)
    }
}