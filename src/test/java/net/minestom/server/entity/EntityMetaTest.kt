...
class EntityMetaTest {
    ...
    fun ensureRegistration() {
        val list: MutableList<String> = ArrayList()
        for (field in EntityTypes::class.java.declaredFields) {
            val entityType = field[this] as EntityType
            val name = entityType.name()
            if (EntityTypeImpl.ENTITY_META_SUPPLIER[name] == null) {
                list.add(name)
            }
        }
        Assertions.assertTrue(list.isEmpty(), "Missing meta for: $list")
    }
}