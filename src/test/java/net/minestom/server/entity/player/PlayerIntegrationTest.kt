...
class PlayerIntegrationTest {
     ...
     */
    @Test
    fun gamemodeTest(env: Env) {
        val instance = env.createFlatInstance()
        val connection = env.createConnection()
        val player = connection.connect(instance, Pos(0, 42, 0)).join()
        Assertions.assertEquals(instance, player.instance)
    ...
    private fun assertAbilities(
        player: Player, isInvulnerable: Boolean, isFlying: Boolean, isAllowFlying: Boolean,
        isInstantBreak: Boolean
    ) {
        Assertions.assertEquals(isInvulnerable, player.isInvulnerable)
        Assertions.assertEquals(isFlying, player.isFlying)
        Assertions.assertEquals(isAllowFlying, player.isAllowFlying)
        Assertions.assertEquals(isInstantBreak, player.isInstantBreak)
    }

    @Test
    fun playerJoinPackets(env: Env) {
        val instance = env.createFlatInstance()
        val connection = env.createConnection()
        val packets: List<Class<out Record?>> = java.util.List.of(
            JoinGamePacket::class.java, ServerDifficultyPacket::class.java, SpawnPositionPacket::class.java,
            DeclareCommandsPacket::class.java, EntityPropertiesPacket::class.java, EntityStatusPacket::class.java,
            UpdateHealthPacket::class.java, PlayerAbilitiesPacket::class.java
        )
        val trackers: MutableList<Collector<*>> = ArrayList()
        for (packet in packets) {
            trackers.add(connection.trackIncoming(packet))
        }
        val trackerAll = connection.trackIncoming(
            ServerPacket::class.java
        )
        val player = connection.connect(instance, Pos(0, 40, 0)).join()
        Assertions.assertEquals(instance, player.instance)
        Assertions.assertEquals(Pos(0, 40, 0), player.position)
        for (tracker in trackers) {
            Assertions.assertEquals(1, tracker.collect().size)
        }
        Assertions.assertTrue(trackerAll.collect().size > packets.size)
    }
     ...
     */
    @Test
    fun refreshPlayerTest(env: Env) {
        val TEST_PERMISSION_LEVEL = 2
        val testDimension = builder(NamespaceID.from("minestom:test_dimension")).build()
        env.process().dimension().addDimension(testDimension)
        val instance = env.createFlatInstance()
        val instance2 = env.process().instance().createInstanceContainer(testDimension)
        val connection = env.createConnection()
        val player = connection.connect(instance, Pos(0, 42, 0)).join()
        Assertions.assertEquals(instance, player.instance)
        val tracker1 = connection.trackIncoming(
            UpdateHealthPacket::class.java
        )
        val tracker2 = connection.trackIncoming(
            SetExperiencePacket::class.java
        )
        val trackerStatus = connection.trackIncoming(
            EntityStatusPacket::class.java
        )
        val tracker4 = connection.trackIncoming(
            PlayerAbilitiesPacket::class.java
        )
        player.permissionLevel = TEST_PERMISSION_LEVEL

        // #join may cause the thread to hang as scheduled for the next tick when initially in a pool
        Assertions.assertTimeout<Void>(Duration.ofSeconds(2)) { player.setInstance(instance2).join() }
        Assertions.assertEquals(instance2, player.instance)
        Assertions.assertEquals(1, tracker1.collect().size)
        Assertions.assertEquals(1, tracker2.collect().size)
        Assertions.assertEquals(2, trackerStatus.collect().size)
        Assertions.assertEquals(1, tracker4.collect().size)

        // Ensure that the player was sent the permission levels
        for (statusPacket in trackerStatus.collect()) {
            Assertions.assertEquals(player.entityId, statusPacket.entityId())
            Assertions.assertEquals(
                24 + TEST_PERMISSION_LEVEL,
                statusPacket.status().toInt()
            ) // TODO: Remove magic value of 24
        }
    }
}