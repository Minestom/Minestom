...
class EntityViewerRuleIntegrationTest {
    @Test
    fun viewableRule(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 0))
        p1.updateViewableRule { p: Player -> p.entityId == p1.entityId + 1 }
        ...
        p1.updateViewableRule { player: Player? -> false }
        Assertions.assertEquals(0, p1.getViewers().size)
        Assertions.assertEquals(1, p2.getViewers().size)
    }

    @Test
    fun viewableRuleUpdate(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 0))
        val enabled = AtomicBoolean(false)
        p1.updateViewableRule { p: Player? -> enabled.get() }
        val p2 = env.createPlayer(instance, Pos(0, 42, 0))
        Assertions.assertEquals(0, p1.getViewers().size)
        Assertions.assertEquals(1, p2.getViewers().size)
        enabled.set(true)
    ...
    @Test
    fun viewableRuleDouble(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 0))
        val p2 = env.createPlayer(instance, Pos(0, 42, 0))
        Assertions.assertEquals(1, p1.getViewers().size)
        Assertions.assertEquals(1, p2.getViewers().size)
        val enabled1 = AtomicBoolean(false)
        val enabled2 = AtomicBoolean(false)
        p1.updateViewableRule { p: Player? -> enabled1.get() }
        p2.updateViewableRule { p: Player? -> enabled2.get() }
        Assertions.assertEquals(0, p1.getViewers().size)
    ...
    @Test
    fun viewerRule(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 0))
        p1.updateViewerRule { e: Entity -> e.entityId == p1.entityId + 1 }
        val p2 = env.createPlayer(instance, Pos(0, 42, 0))
        Assertions.assertEquals(1, p1.getViewers().size)
        Assertions.assertEquals(1, p2.getViewers().size)
        p1.updateViewerRule { player: Entity? -> false }
        Assertions.assertEquals(1, p1.getViewers().size)
        Assertions.assertEquals(0, p2.getViewers().size)
    }

    @Test
    fun viewerRuleUpdate(env: Env) {
        val instance = env.createFlatInstance()
        val p1 = env.createPlayer(instance, Pos(0, 42, 0))
        val enabled = AtomicBoolean(false)
        p1.updateViewerRule { e: Entity? -> enabled.get() }
        val p2 = env.createPlayer(instance, Pos(0, 42, 0))
        Assertions.assertEquals(1, p1.getViewers().size)
        Assertions.assertEquals(0, p2.getViewers().size)
        enabled.set(true)
    ...
    fun viewerRuleDouble(env: Env) {
        ...
        val enabled1 = AtomicBoolean(false)
        val enabled2 = AtomicBoolean(false)
        p1.updateViewerRule { e: Entity? -> enabled1.get() }
        p2.updateViewerRule { e: Entity? -> enabled2.get() }
        Assertions.assertEquals(0, p1.getViewers().size)
        Assertions.assertEquals(0, p2.getViewers().size)
        enabled1.set(true)
        p1.updateViewerRule()
