...
class BlockPropertiesTest {
    @Test
    fun empty() {
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties("[]"))
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties(""))
    }

    @Test
    fun noBrackets() {
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties("random test without brackets"))
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties("["))
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties("[end"))
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties("[random test without end bracket"))
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties("]"))
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties("start]"))
        Assertions.assertEquals(Map.of<Any, Any>(), BlockUtils.parseProperties("random test without start bracket]"))
    }
    ...
    @Test
    fun single() {
        Assertions.assertEquals(Map.of("facing", "east"), BlockUtils.parseProperties("[facing=east]"))
    }

    @Test
    fun doubleSpace() {
        Assertions.assertEquals(
            Map.of("facing", "east", "key", "value"),
            BlockUtils.parseProperties("[facing=east,key=value ]")
        )
        Assertions.assertEquals(
            Map.of("facing", "east", "key", "value"),
            BlockUtils.parseProperties("[ facing = east, key= value ]")
    ...
    fun allLengths() {
        ...
        for (i in 0..12) {
            ...
            properties.append("]")
            val map = BlockUtils.parseProperties(properties.toString())
            Assertions.assertEquals(i, map.size)
            for (j in 0 until i) {
                Assertions.assertEquals("value$j", map["key$j"])
            }
        }
    }
