...
class InstanceBlockIntegrationTest {
    @Test
    fun replaceAir(env: Env) {
        val instance = env.createFlatInstance()
        val connection = env.createConnection()
        connection.connect(instance, Pos(0, 40, 0)).join()
        val blockPoint = Vec(5, 41, 0)
        Assertions.assertEquals(Block.AIR, instance.getBlock(blockPoint))
        val tracker = connection.trackIncoming()
        instance.setBlock(blockPoint, Block.STONE)
        tracker.assertSingle(BlockChangePacket::class.java, Consumer { packet: BlockChangePacket? ->
            Assertions.assertEquals(blockPoint, packet!!.blockPosition())
            Assertions.assertEquals(Block.STONE.stateId().toInt(), packet.blockStateId())
        })
        Assertions.assertEquals(Block.STONE, instance.getBlock(blockPoint))
    }

    @Test
    fun placeBlockEntity(env: Env) {
        val instance = env.createFlatInstance()
        val connection = env.createConnection()
        connection.connect(instance, Pos(0, 40, 0)).join()
        val blockPoint = Vec(5, 41, 0)
        val signHandler: BlockHandler = object : BlockHandler {
        ...
        try {
            data = SNBTParser(
                StringReader(
                    "{\"GlowingText\":0B,\"Color\":\"black\",\"Text1\":\"{\\\"text\\\":\\\"wawsd\\\"}\"," +
        ...
        val blockChangeTracker = connection.trackIncoming(
            BlockChangePacket::class.java
        )
        val blockEntityTracker = connection.trackIncoming(
            BlockEntityDataPacket::class.java
        )
        instance.setBlock(blockPoint, block)
        blockChangeTracker.assertSingle { packet: BlockChangePacket? ->
            Assertions.assertEquals(blockPoint, packet!!.blockPosition())
            Assertions.assertEquals(block.stateId().toInt(), packet.blockStateId())
        }
        blockEntityTracker.assertSingle { packet: BlockEntityDataPacket? ->
            Assertions.assertEquals(blockPoint, packet!!.blockPosition())
            Assertions.assertEquals(block.registry().blockEntityId(), packet.action())
            Assertions.assertEquals(data, packet.data())
        }
        Assertions.assertEquals(block, instance.getBlock(blockPoint))
    }
}