...
class BlockClientNbtTest {
    @Test
    fun basic() {
        Assertions.assertNull(BlockUtils.extractClientNbt(Block.STONE))
        Assertions.assertNull(BlockUtils.extractClientNbt(Block.GRASS))
        Assertions.assertEquals(NBTCompound.EMPTY, BlockUtils.extractClientNbt(Block.CHEST))
        val nbt = NBT.Compound(Map.of("test", NBT.String("test")))
        Assertions.assertEquals(nbt, BlockUtils.extractClientNbt(Block.CHEST.withNbt(nbt)))
    }
    ...
    fun handler() {
        val handler: BlockHandler = object : BlockHandler {
            override fun getBlockEntityTags(): Collection<Tag<*>> {
                return List.of<Tag<*>>(Tag.String("test"))
            }
        ...
        val nbt = NBT.Compound(Map.of("test", NBT.String("test")))
        Assertions.assertNull(BlockUtils.extractClientNbt(Block.STONE.withNbt(nbt).withHandler(handler)))
        Assertions.assertEquals(nbt, BlockUtils.extractClientNbt(Block.CHEST.withNbt(nbt).withHandler(handler)))
        Assertions.assertEquals(
            nbt, BlockUtils.extractClientNbt(
                Block.CHEST
