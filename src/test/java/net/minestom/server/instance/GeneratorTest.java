...
public class GeneratorTest {
    ...
    public void chunkSize() {
        ...
        Arrays.setAll(sections, i -> Section.inMemory());
    ...
    public void chunkAbsolute() {
        ...
        assertEquals(Block.STONE, sections[0].getBlock(8, 0, 7));
        assertEquals(Block.STONE, sections[1].getBlock(8, 1, 7));
    }
    ...
    public void chunkAbsoluteAll() {
        ...
        for (var section : sections) {
            section.forEachBlock((x, y, z, block) -> assertEquals(Block.STONE, block));
        }
    }
    ...
    public void chunkRelative() {
        ...
        generator.generate(chunkUnits);
        assertEquals(Block.STONE, sections[0].getBlock(0, 0, 0));
        assertEquals(Block.STONE, sections[1].getBlock(0, 0, 2));
        assertEquals(Block.STONE, sections[2].getBlock(5, 1, 5));
    }
    ...
    public void chunkBiomeSet() {
        ...
        generator.generate(chunkUnits);
        assertEquals(Biome.PLAINS.id(), sections[0].getBiome(0, 0, 0).id());
        assertEquals(0, sections[0].getBiome(1, 0, 0).id());
        assertEquals(Biome.PLAINS.id(), sections[0].getBiome(2, 0, 0).id());
    }
    ...
    public void chunkBiomeFill() {
        ...
        generator.generate(chunkUnits);
        for (var section : sections) {
            section.forEachBiome((x, y, z, biome) -> assertEquals(Biome.PLAINS, biome));
        }
    }
        ...
        for (var section : sections) {
            section.forEachBlock((x, y, z, block) -> {
                if (index.get() == 0 || index.get() == 1) {
                    assertEquals(Block.STONE, block, "filling failed for section " + index.get());
                } else {
    ...
    public void chunkFillHeightOneOff() {
        ...
        for (var section : sections) {
            section.forEachBlock((x, y, z, block) -> {
                ...
                assertEquals(expected, block, "fail for coordinate: " + x + "," + y + "," + z + " for index " + index.get());
            });
            index.incrementAndGet();
        }
    }
    ...
    public void sectionFill() {
        Section section = Section.inMemory();
