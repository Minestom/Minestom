...
class BlockTest {
    @Test
    fun testNBT() {
        var block: Block = Block.CHEST
        Assertions.assertFalse(block.hasNbt())
        Assertions.assertNull(block.nbt())
        val nbt = NBTCompound(Map.of("key", NBT.Int(5)))
        block = block.withNbt(nbt)
        Assertions.assertTrue(block.hasNbt())
        Assertions.assertEquals(block.nbt(), nbt)
        block = block.withNbt(null)
        Assertions.assertFalse(block.hasNbt())
        Assertions.assertNull(block.nbt())
        val value = block.getTag(Tag.String("key").defaultValue("Default"))
    ...
    fun validProperties() {
        ...
        Assertions.assertEquals(block.properties(), Objects.requireNonNull(Block.fromBlockId(block.id())).properties())
        ...
        for (possible in block.possibleStates()) {
            Assertions.assertEquals(possible, block.withProperties(possible.properties()))
        }
        Assertions.assertEquals(block.withProperty("facing", "north").getProperty("facing"), "north")
        Assertions.assertNotEquals(block.withProperty("facing", "north"), block.withProperty("facing", "south"))
    }

    @Test
    fun invalidProperties() {
        val block: Block = Block.CHEST
        Assertions.assertThrows(Exception::class.java) { block.withProperty("random", "randomKey") }
        Assertions.assertThrows(Exception::class.java) { block.withProperties(Map.of("random", "randomKey")) }
    }

    @Test
    fun testEquality() {
        val nbt = NBTCompound(Map.of("key", NBT.Int(5)))
        val b1: Block = Block.CHEST
        val b2: Block = Block.CHEST
        Assertions.assertEquals(b1.withNbt(nbt), b2.withNbt(nbt))
        Assertions.assertEquals(b1.withProperty("facing", "north").getProperty("facing"), "north")
        Assertions.assertEquals(b1.withProperty("facing", "north"), b2.withProperty("facing", "north"))
    }

    @Test
    fun testMutability() {
        val block: Block = Block.CHEST
        Assertions.assertThrows(Exception::class.java) { block.properties()["facing"] = "north" }
        Assertions.assertThrows(Exception::class.java) {
            block.withProperty("facing", "north").properties()["facing"] = "south"
        }
    }

    @Test
    fun testShape() {
        val start: Point = Block.LANTERN.registry().collisionShape().relativeStart()
        val end: Point = Block.LANTERN.registry().collisionShape().relativeEnd()
        Assertions.assertEquals(start, Vec(0.312, 0, 0.312))
        Assertions.assertEquals(end, Vec(0.625, 0.437, 0.625))
    }
}