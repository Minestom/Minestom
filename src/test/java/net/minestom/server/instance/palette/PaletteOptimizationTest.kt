...
class PaletteOptimizationTest {
    ...
    fun random() {
        ...
        val palette = createPalette()
        palette.setAll { x: Int, y: Int, z: Int -> random.nextInt(256) }
        paletteEquals(palette.palette, palette.optimizedPalette())
        palette.setAll { x: Int, y: Int, z: Int -> random.nextInt(2) }
        paletteEquals(palette.palette, palette.optimizedPalette())
    }
    ...
    fun manualFill() {
        ...
        paletteEquals(palette.palette, palette.optimizedPalette())
        palette.setAll { x: Int, y: Int, z: Int -> 2 }
        paletteEquals(palette.palette, palette.optimizedPalette())
        palette.setAll { x: Int, y: Int, z: Int -> 0 }
        paletteEquals(palette.palette, palette.optimizedPalette())
    }

    fun createPalette(): AdaptivePalette {
        return Palette.blocks() as AdaptivePalette
    }

    fun paletteEquals(palette: Palette, optimized: Palette) {
        // Verify content
        Assertions.assertEquals(palette.dimension(), optimized.dimension())
        for (y in 0 until palette.dimension()) {
            for (z in 0 until palette.dimension()) {
                for (x in 0 until palette.dimension()) {
                    Assertions.assertEquals(palette[x, y, z], optimized[x, y, z])
                }
            }
        }
        // Verify size
        run {
            var writer = BinaryWriter(4096)
            palette.write(writer)
            val length1 = writer.toByteArray().size
            writer = BinaryWriter(4096)
            optimized.write(writer)
            val length2 = writer.toByteArray().size

            //System.out.println("debug: " + Thread.currentThread().getStackTrace()[2].getMethodName() + " " + length1 + " " + length2);
            Assertions.assertTrue(
                length1 >= length2,
                "Optimized palette is bigger than the original one: $length1 : $length2"
            )
        }
    }
}