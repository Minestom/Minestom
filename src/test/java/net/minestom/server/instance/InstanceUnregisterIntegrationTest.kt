...
class InstanceUnregisterIntegrationTest {
    @Test
    fun sharedInstance(env: Env) {
        // Ensure that unregistering a shared instance does not unload the container chunks
        val instanceManager = env.process().instance()
        val instance = instanceManager.createInstanceContainer()
        val shared1 = instanceManager.createSharedInstance(instance)
        val connection = env.createConnection()
        val player = connection.connect(shared1, Pos(0, 40, 0)).join()
        val listener: FlexibleListener<PlayerTickEvent?> = env.listen<PlayerTickEvent>(PlayerTickEvent::class.java)!!
        listener.followup()
        env.tick()
