...
import net.kyori.adventure.bossbar.BossBarimport
...
import net.minestom.server.item.ItemMetaimport

org.junit.jupiter.api.Assertionsimport org.junit.jupiter.api.Test
import regressions.ItemMetaBuilderRegressions.BasicMetaBuilderimport

java.lang.Exceptionimport java.util.Listimport java.util.Mapimport java.util.stream.Stream
class ItemTest {
    @Test
    fun testFields() {
        var item = ItemStack.of(Material.DIAMOND_SWORD)
        Assertions.assertEquals(item.material, Material.DIAMOND_SWORD, "Material must be the same")
        Assertions.assertEquals(item.amount, 1, "Default item amount must be 1")
        Assertions.assertNull(item.displayName, "Default item display name must be null")
        Assertions.assertTrue(item.lore.isEmpty(), "Default item lore must be empty")
        val finalItem = item
        Assertions.assertThrows(
            Exception::class.java,
            { finalItem.lore.add(Component.text("Hey!")) },
            "Lore list cannot be modified directly"
        )
        item = item.withAmount(5)
        Assertions.assertEquals(item.amount, 5, "Items with different amount should not be equals")
        Assertions.assertEquals(
            item.withAmount { amount: Int -> amount * 2 }.amount,
            10,
            "Amount must be multiplied by 2"
        )
    }

    @Test
    fun testEquality() {
        val item1 = ItemStack.of(Material.DIAMOND_SWORD)
        val item2 = ItemStack.of(Material.DIAMOND_SWORD)
        Assertions.assertEquals(item1, item2)
        Assertions.assertNotEquals(item1.withAmount(5), item2.withAmount(2))
        Assertions.assertTrue(item1.isSimilar(item2))
        Assertions.assertTrue(item1.withAmount(5).isSimilar(item2.withAmount(2)))
        Assertions.assertFalse(item1.isSimilar(item2.withDisplayName(Component.text("Hey!"))))
    }

    @Test
    fun testAir() {
        val item = ItemStack.of(Material.DIAMOND_SWORD)
        Assertions.assertFalse(item.isAir)
        Assertions.assertTrue(ItemStack.AIR.isAir)
        val emptyItem = item.withAmount(0)
        Assertions.assertTrue(emptyItem.isAir)
        Assertions.assertEquals(emptyItem, ItemStack.AIR, "AIR item can be compared to empty item")
        Assertions.assertSame(emptyItem, ItemStack.AIR, "AIR item identity can be compared to empty item")
    }

    @Test
    fun testItemNbt() {
        val itemNbt = ITEM.toItemNBT()
        Assertions.assertEquals(itemNbt.getString("id"), ITEM.material.name(), "id string should be the material name")
        Assertions.assertEquals(itemNbt.getByte("Count"), ITEM.amount.toByte(), "Count byte should be the item amount")
        val metaNbt = itemNbt.getCompound("tag")
        val metaNbt2 = ITEM.meta.toNBT()
        Assertions.assertEquals(metaNbt, metaNbt2, "tag compound should be equal to the meta nbt")
    }
    ...
    fun testFromNbt() {
        ...
        Assertions.assertEquals(ITEM, item, "Items must be equal if created from the same item nbt")
        Assertions.assertEquals(itemNbt, item.toItemNBT(), "Item nbt must be equal back")
        val metaNbt = ITEM.meta.toNBT()
        item = ItemStack.fromNBT(ITEM.material, metaNbt, ITEM.amount)
        Assertions.assertEquals(ITEM, item, "Items must be equal if created from the same meta nbt")
    }

    @Test
    fun testEnchant() {
        var item = ItemStack.of(Material.DIAMOND_SWORD)
        var enchantments = item.meta.enchantmentMap
        Assertions.assertTrue(enchantments.isEmpty(), "items do not have enchantments by default")
        item = item.withMeta { meta: ItemMetaBuilder -> meta.enchantment(Enchantment.EFFICIENCY, 10.toShort()) }
        enchantments = item.meta.enchantmentMap
        Assertions.assertEquals(enchantments.size, 1)
        Assertions.assertEquals(enchantments[Enchantment.EFFICIENCY], 10.toShort())
        item = item.withMeta { meta: ItemMetaBuilder -> meta.enchantment(Enchantment.INFINITY, 5.toShort()) }
        enchantments = item.meta.enchantmentMap
        Assertions.assertEquals(enchantments.size, 2)
        Assertions.assertEquals(enchantments[Enchantment.EFFICIENCY], 10.toShort())
        Assertions.assertEquals(enchantments[Enchantment.INFINITY], 5.toShort())
        item = item.withMeta { meta: ItemMetaBuilder -> meta.enchantments(Map.of()) }
        enchantments = item.meta.enchantmentMap
        Assertions.assertTrue(enchantments.isEmpty())

        // Ensure that enchantments can still be modified after being emptied
        item = item.withMeta { meta: ItemMetaBuilder -> meta.enchantment(Enchantment.EFFICIENCY, 10.toShort()) }
        enchantments = item.meta.enchantmentMap
        Assertions.assertEquals(enchantments[Enchantment.EFFICIENCY], 10.toShort())
        item = item.withMeta { obj: ItemMetaBuilder -> obj.clearEnchantment() }
        enchantments = item.meta.enchantmentMap
        Assertions.assertTrue(enchantments.isEmpty())
    }
    ...
    fun testLore() {
        var item = ItemStack.of(Material.DIAMOND_SWORD)
        Assertions.assertEquals(List.of<Any>(), item.lore)
        Assertions.assertNull(item.meta.toNBT()["display"])
        run {
            val lore = List.of(Component.text("Hello"))
            item = item.withLore(lore)
            Assertions.assertEquals(lore, item.lore)
            val loreNbt = item.meta.toNBT().getCompound("display")!!.getList<NBTString>("Lore")!!
            Assertions.assertNotNull(loreNbt)
            Assertions.assertEquals(loreNbt.size, 1)
            Assertions.assertEquals(
                lore,
                loreNbt.asListView().stream()
                    .map { line: NBTString -> GsonComponentSerializer.gson().deserialize(line.value) }
                    .toList())
        }
        run {
            val lore = List.of(Component.text("Hello"), Component.text("World"))
            item = item.withLore(lore)
            Assertions.assertEquals(lore, item.lore)
            val loreNbt = item.meta.toNBT().getCompound("display")!!.getList<NBTString>("Lore")!!
            Assertions.assertNotNull(loreNbt)
            Assertions.assertEquals(loreNbt.size, 2)
            Assertions.assertEquals(
                lore,
                loreNbt.asListView().stream()
                    .map { line: NBTString -> GsonComponentSerializer.gson().deserialize(line.value) }
        ...
        run {
            val lore = Stream.of("string test").map { content: String? ->
                Component.text(
                    content!!
                )
            }.toList()
    ...
    fun testBuilderReuse() {
        val builder = ItemStack.builder(Material.DIAMOND)
        val item1 = builder.build()
        val item2 = builder.displayName(Component.text("Name")).build()
        Assertions.assertNull(item1.displayName)
        Assertions.assertNotNull(item2.displayName)
        Assertions.assertNotEquals(item1, item2, "Item builder should be reusable")
    }

    companion object {
        private val ITEM = ItemStack.builder(Material.STONE)
