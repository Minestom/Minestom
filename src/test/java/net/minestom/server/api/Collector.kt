...
interface Collector<T> {
    ...
    fun assertEmpty() {
        ...
        Assertions.assertEquals(0, elements.size, "Expected 0 element, got $elements")
    }

    fun <P : T?> assertSingle(type: Class<P>, consumer: Consumer<P>) {
        val elements = collect()
        Assertions.assertEquals(1, elements.size, "Expected 1 element, got $elements")
        val element = elements[0]
        Assertions.assertInstanceOf(
            type,
            element,
            "Expected type " + type.simpleName + ", got " + element.javaClass.getSimpleName()
        )
        consumer.accept(element as P)
    }

    fun assertSingle(consumer: Consumer<T>) {
        val elements = collect()
        Assertions.assertEquals(1, elements.size, "Expected 1 element, got $elements")
        consumer.accept(elements[0])
    }
}