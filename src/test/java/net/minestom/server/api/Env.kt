...
interface Env {
    fun process(): ServerProcess
    fun createConnection(): TestConnection
    fun <E : Event?, H> trackEvent(eventType: Class<E>, filter: EventFilter<in E, H>, actor: H): Collector<E>
    fun <E : Event?> listen(eventType: Class<E>): FlexibleListener<E>
    fun tick() {
        process().ticker().tick(System.nanoTime())
    }

    fun tickWhile(condition: BooleanSupplier, timeout: Duration?): Boolean {
        val ticker = process().ticker()
    ...
    fun createPlayer(instance: Instance, pos: Pos): Player {
        return createConnection().connect(instance, pos).join()
    }

    fun createFlatInstance(): Instance {
        val instance = process().instance().createInstanceContainer()
        instance.chunkGenerator = object : ChunkGenerator {
