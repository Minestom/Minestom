...
object TestUtils {
    ...
    fun assertEqualsSNBT(snbt: String?, compound: NBTCompound?) {
        try {
            val converted = SNBTParser(StringReader(snbt)).parse() as NBTCompound
            Assertions.assertEquals(converted, compound)
    ...
    @JvmOverloads
    fun assertEqualsIgnoreSpace(s1: String, s2: String, matchCase: Boolean = true) {
        val val1 = stripExtraSpaces(s1)
        val val2 = stripExtraSpaces(s2)
        if (matchCase) {
            Assertions.assertEquals(val1, val2)
        } else {
            Assertions.assertTrue(val1.equals(val2, ignoreCase = true))
        }
    }

    private fun stripExtraSpaces(s: String): String {
