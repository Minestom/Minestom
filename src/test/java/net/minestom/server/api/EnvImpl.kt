...
internal class EnvImpl(private val process: ServerProcess) : Env {
    private val listeners: MutableList<FlexibleListenerImpl<*>> = CopyOnWriteArrayList()
    override fun process(): ServerProcess {
        return process
    }

    override fun createConnection(): TestConnection {
        return TestConnectionImpl(this)
    }

    override fun <E : Event?, H> trackEvent(eventType: Class<E>, filter: EventFilter<in E, H>, actor: H): Collector<E> {
        val tracker = EventCollector<E>(actor)
        process.eventHandler().map(actor, filter).addListener(eventType) { e: E -> tracker.events.add(e) }
        return tracker
    }

    override fun <E : Event?> listen(eventType: Class<E>): FlexibleListener<E> {
        val handler = process.eventHandler()
        val flexible = FlexibleListenerImpl(eventType)
        val listener = EventListener.of(eventType) { e: E -> flexible.handler.accept(e) }
        handler.addListener(listener)
        listeners.add(flexible)
        return flexible
    }

    fun cleanup() {
        listeners.forEach(Consumer { obj: FlexibleListenerImpl<*> -> obj.check() })
    }

    internal inner class EventCollector<E : Event?>(private val handler: Any) : Collector<E> {
        val events: MutableList<E> = CopyOnWriteArrayList()
        override fun collect(): List<E> {
            process.eventHandler().unmap(handler)
            return java.util.List.copyOf(events)
        }
    }

    internal class FlexibleListenerImpl<E : Event?>(private val eventType: Class<E>) : FlexibleListener<E> {
        var handler = Consumer { e: E -> }
        private var initialized = false
        private var called = false
        override fun followup(handler: Consumer<E>) {
            updateHandler(handler)
        }

        override fun failFollowup() {
            updateHandler { e: E -> Assertions.fail<Any>("Event " + e.javaClass.getSimpleName() + " was not expected") }
        }

        fun updateHandler(handler: Consumer<E>) {
            check()
            initialized = true
            called = false
            this.handler = Consumer { e: E ->
                handler.accept(e)
                called = true
            }
        }
        ...
        fun check() {
            Assertions.assertTrue(!initialized || called, "Last listener has not been called: " + eventType.simpleName)
        }
    }
}