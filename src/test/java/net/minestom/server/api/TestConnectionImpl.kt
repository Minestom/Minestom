...
internal class TestConnectionImpl(private val env: Env) : TestConnection {
    private val process: ServerProcess
    private val playerConnection = PlayerConnectionImpl()
    private val incomingTrackers: MutableList<IncomingCollector<ServerPacket>> = CopyOnWriteArrayList()

    init {
        process = env.process()
    }

    override fun connect(instance: Instance, pos: Pos): CompletableFuture<Player> {
        val player = Player(UUID.randomUUID(), "RandName", playerConnection)
        player.eventNode().addListener(PlayerLoginEvent::class.java) { event: PlayerLoginEvent ->
    ...
    override fun <T : ServerPacket?> trackIncoming(type: Class<T>): Collector<T> {
        val tracker = IncomingCollector(type)
        incomingTrackers.add(IncomingCollector::class.java.cast(tracker))
        return tracker
    }

    internal inner class PlayerConnectionImpl : PlayerConnection() {
        override fun sendPacket(packet: SendablePacket) {
            for (tracker in incomingTrackers) {
                val serverPacket = SendablePacket.extractServerPacket(packet)
                if (tracker.type.isAssignableFrom(serverPacket.javaClass)) tracker.packets.add(serverPacket)
            }
        }

        override fun getRemoteAddress(): SocketAddress {
            return InetSocketAddress("localhost", 25565)
        }

        override fun disconnect() {}
    }

    internal inner class IncomingCollector<T : ServerPacket?>(val type: Class<T>) : Collector<T> {
        val packets: MutableList<T> = CopyOnWriteArrayList()
        override fun collect(): List<T> {
            incomingTrackers.remove(this)
            return java.util.List.copyOf(packets)
        }
    }
}