...
class EntityBlockTouchTickIntegrationTest {
    @Test
    fun entityPhysicsCheckTouchTick(env: Env) {
        val instance = env.createFlatInstance()
        val positions: MutableSet<Point> = HashSet()
        val handler: BlockHandler = object : BlockHandler {
            override fun onTouch(touch: BlockHandler.Touch) {
                Assertions.assertTrue(positions.add(touch.blockPosition))
            }
    ...
    @Test
    fun entityPhysicsCheckTouchTickFarZ(env: Env) {
        val instance = env.createFlatInstance()
        instance.loadChunk(Pos(1000, 1000, 1000))
        val positions: MutableSet<Point> = HashSet()
        val handler: BlockHandler = object : BlockHandler {
            override fun onTouch(touch: BlockHandler.Touch) {
                Assertions.assertTrue(positions.add(touch.blockPosition))
        ...
        val entity = Entity(EntityType.ZOMBIE)
        entity.setInstance(instance, Pos(1000, 42, 1000.7)).join()
        entity.tick(0)
        Assertions.assertEquals(
            Set.of(
                Vec(1000, 42, 1000),
    ...
    @Test
    fun entityPhysicsCheckTouchTickFarX(env: Env) {
        val instance = env.createFlatInstance()
        instance.loadChunk(Pos(1000, 1000, 1000))
        val positions: MutableSet<Point> = HashSet()
        val handler: BlockHandler = object : BlockHandler {
        ...
        instance.setBlock(1000, 42, 1010, Block.STONE.withHandler(handler))
        val entity = Entity(EntityType.ZOMBIE)
        entity.setInstance(instance, Pos(1000.699, 42, 1000)).join()
        entity.tick(0)
        Assertions.assertEquals(
            Set.of(
                Vec(1000, 43, 999),
    ...
    @Test
    fun entityPhysicsCheckTouchTickFarNegative(env: Env) {
        val instance = env.createFlatInstance()
        instance.loadChunk(Pos(-1000, 44, -1000))
        val positions: MutableSet<Point> = HashSet()
        val handler: BlockHandler = object : BlockHandler {
            override fun onTouch(touch: BlockHandler.Touch) {
                Assertions.assertTrue(positions.add(touch.blockPosition))
        ...
        instance.setBlock(-1000, 42, -1010, Block.STONE.withHandler(handler))
        val entity = Entity(EntityType.ZOMBIE)
        entity.setInstance(instance, Pos(-1000.301, 42, -1000)).join()
        entity.tick(0)
        Assertions.assertEquals(
            Set.of(
                Vec(-1001, 43, -1000),
