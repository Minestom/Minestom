...
class PlacementCollisionIntegrationTest {
    ...
    fun empty(env: Env) {
        ...
        Assertions.assertTrue(BlockCollision.canPlaceBlockAt(instance, Vec(0, 40, 0), Block.STONE))
    }

    @Test
    fun entityBlock(env: Env) {
        val instance = env.createFlatInstance()
        Entity(EntityType.ZOMBIE).setInstance(instance, Pos(0, 40, 0)).join()
        Assertions.assertFalse(BlockCollision.canPlaceBlockAt(instance, Vec(0, 40, 0), Block.STONE))
    }

    @Test
    fun slab(env: Env) {
        val instance = env.createFlatInstance()
        Entity(EntityType.ZOMBIE).setInstance(instance, Pos(0, 40.75, 0)).join()
        Assertions.assertTrue(BlockCollision.canPlaceBlockAt(instance, Vec(0, 40, 0), Block.STONE_SLAB))
    }

    @Test
    fun belowPlayer(env: Env) {
        val instance = env.createFlatInstance()
        env.createPlayer(instance, Pos(5.7, -8, 6.389))
        Assertions.assertTrue(BlockCollision.canPlaceBlockAt(instance, Vec(5, -9, 6), Block.STONE))
    }
}