package net.minestom.server.collisionimport
...
class EntityProjectileCollisionIntegrationTest {
    @Test
    fun blockShootAndBlockRemoval(env: Env) {
        val instance = env.createFlatInstance()
        instance.worldBorder.diameter = 1000.0
        val shooter = Entity(EntityType.SKELETON)
        shooter.setInstance(instance, Pos(0, 40, 0)).join()
        val projectile = EntityProjectile(shooter, EntityType.ARROW)
        projectile.setInstance(instance, shooter.position.withY { y: Double -> y + shooter.eyeHeight }).join()
        val blockPosition: Point = Vec(5, 40, 0)
        val block: Block = Block.GRASS_BLOCK
        instance.setBlock(blockPosition, block)
        projectile.shoot(blockPosition, 1.0, 0.0)
        val eventRef = AtomicReference<ProjectileCollideWithBlockEvent?>()
        globalEventHandler.addListener(ProjectileCollideWithBlockEvent::class.java) { newValue: ProjectileCollideWithBlockEvent? ->
            eventRef.set(
                newValue
            )
        }
        val tick = TimeUnit.SERVER_TICK.duration.toMillis()
        for (i in 0 until MinecraftServer.TICK_PER_SECOND) {
        ...
        Assertions.assertEquals(
            blockPosition, Vec(
                event!!.collisionPosition.blockX().toDouble(),
                event.collisionPosition.blockY().toDouble(),
                event.collisionPosition.blockZ()
                    .toDouble()
            )
        )
        Assertions.assertEquals(block, event.block)
        val eventRef2 = AtomicReference<ProjectileUncollideEvent>()
        globalEventHandler.addListener(ProjectileUncollideEvent::class.java) { newValue: ProjectileUncollideEvent ->
            eventRef2.set(
                newValue
            )
        }
        ...
        event = eventRef.get()
        val event2 = eventRef2.get()
        Assertions.assertNotNull(event)
        Assertions.assertNotNull(event2)
        Assertions.assertEquals(
            blockPosition.withY { y: Double -> y - 1 }, Vec(
                event!!.collisionPosition.blockX().toDouble(),
                event.collisionPosition.blockY().toDouble(),
                event.collisionPosition.blockZ()
                    .toDouble()
    ...
    fun entityShoot(env: Env) {
        ...
        var dx = 1.0
        while (dx <= 3) {
            singleEntityShoot(instance, shooter, Vec(dx, 40, 0))
            dx += .2
        }
    }

    private fun singleEntityShoot(
        instance: Instance,
        shooter: Entity,
        ...
        val projectile = EntityProjectile(shooter, EntityType.ARROW)
        projectile.setInstance(instance, shooter.position.withY { y: Double -> y + shooter.eyeHeight }).join()
        val target = LivingEntity(EntityType.RABBIT)
        target.setInstance(instance, Pos.fromPoint(targetPosition)).join()
        projectile.shoot(targetPosition, 1.0, 0.0)
        val eventRef = AtomicReference<ProjectileCollideWithEntityEvent>()
        val eventNode = EventNode.all("projectile-test")
        eventNode.addListener(ProjectileCollideWithEntityEvent::class.java) { event: ProjectileCollideWithEntityEvent ->
            event.entity.remove()
            eventRef.set(event)
        ...
        Assertions.assertNotNull(event, "Could not hit entity at $targetPosition")
        Assertions.assertSame(target, event.target)
        Assertions.assertTrue(projectile.boundingBox.intersectEntity(event.collisionPosition, target))
        target.remove()
    }

    @Test
    fun entitySelfShoot(env: Env) {
        val instance = env.createFlatInstance()
        instance.worldBorder.diameter = 1000.0
        val shooter = LivingEntity(EntityType.SKELETON)
        shooter.setInstance(instance, Pos(0, 40, 0)).join()
        val projectile = EntityProjectile(shooter, EntityType.ARROW)
        projectile.setInstance(instance, shooter.position.withY { y: Double -> y + shooter.eyeHeight }).join()
        projectile.shoot(Vec(0, 60, 0), 1.0, 0.0)
        val eventRef = AtomicReference<ProjectileCollideWithEntityEvent>()
        globalEventHandler.addListener(
            ProjectileCollideWithEntityEvent::class.java
        ) { event: ProjectileCollideWithEntityEvent ->
            event.entity.remove()
            eventRef.set(event)
        }
        val tick = TimeUnit.SERVER_TICK.duration.toMillis()
        for (i in 0 until MinecraftServer.TICK_PER_SECOND * 5) {
            if (!projectile.isRemoved) {
                projectile.tick(i * tick)
            }
        }
        val event = eventRef.get()
        Assertions.assertNotNull(event)
        Assertions.assertSame(shooter, event.target)
        Assertions.assertTrue(shooter.boundingBox.intersectEntity(shooter.position, projectile))
    }
}