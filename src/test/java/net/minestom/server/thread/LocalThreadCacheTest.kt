...
class LocalThreadCacheTest {
    @Test
    @Throws(InterruptedException::class)
    fun testLocalThreadCache() {
        val result = AtomicBoolean(false)
        val thread: MinestomThread = object : MinestomThread("name") {
            override fun run() {
                val dummy = -1
                Assertions.assertEquals(7, localCache(1) { 7 })
                Assertions.assertEquals(7, localCache(1) { dummy })
                Assertions.assertEquals(5, localCache(0) { 5 })
                Assertions.assertEquals(7, localCache(1) { dummy })
                Assertions.assertEquals(5, localCache(0) { dummy })
                Assertions.assertEquals(5, localCache(2) { 5 })
                Assertions.assertEquals(7, localCache(1) { dummy })
                Assertions.assertEquals(5, localCache(0) { dummy })
                result.set(true)
            }
        ...
        thread.start()
        thread.join(1500)
        Assertions.assertTrue(result.get(), "Thread didn't start")
    }
}