...
class AcquirableTest {
    @Test
    fun assignation() {
        val tickThread = AtomicReference<TickThread?>()
        val entity: Entity = object : Entity(EntityType.ZOMBIE) {
            override fun tick(time: Long) {
                super.tick(time)
                tickThread.set(getAcquirable<Entity>().assignedThread())
            }
        }
        val first = Any()
        val second = Any()
        val dispatcher = ThreadDispatcher.of(ThreadProvider.counter<Any>(), 2)
        ...
        val firstThread = tickThread.get()
        Assertions.assertNotNull(firstThread)
        tickThread.set(null)
        dispatcher.updateElement(entity, second)
        dispatcher.updateAndAwait(System.currentTimeMillis())
        val secondThread = tickThread.get()
        Assertions.assertNotNull(secondThread)
        Assertions.assertNotEquals(firstThread, secondThread)
    }
}