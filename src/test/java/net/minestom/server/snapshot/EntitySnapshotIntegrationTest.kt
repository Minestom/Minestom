...
class EntitySnapshotIntegrationTest {
    ...
    fun basic(env: Env) {
        ...
        val ent = Entity(EntityType.ZOMBIE)
        ent.setInstance(instance).join()
        val snapshot = ServerSnapshot.update()
        val inst = snapshot.instances().iterator().next()
        val entities = inst.entities()
        Assertions.assertEquals(1, entities.size)
        val entity = entities.iterator().next()
        Assertions.assertEquals(EntityType.ZOMBIE, entity.type())
        Assertions.assertEquals(ent.uuid, entity.uuid())
        Assertions.assertEquals(ent.entityId, entity.id())
        Assertions.assertEquals(ent.position, entity.position())
        Assertions.assertEquals(ent.velocity, entity.velocity())
        Assertions.assertEquals(inst, entity.instance())
        Assertions.assertEquals(inst.chunkAt(entity.position()), entity.chunk())
        Assertions.assertEquals(ent.viewers.size, entity.viewers().size)
        Assertions.assertEquals(ent.passengers.size, entity.passengers().size)
        Assertions.assertNull(ent.vehicle)
        Assertions.assertNull(entity.vehicle())
    }
}