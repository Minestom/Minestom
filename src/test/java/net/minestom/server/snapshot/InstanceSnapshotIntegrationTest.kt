...
class InstanceSnapshotIntegrationTest {
    ...
    fun basic(env: Env) {
        ...
        run {
            var instances: Collection<InstanceSnapshot?> = snapshot.instances()
        ...
        Assertions.assertEquals(snapshot, inst.server(), "Instance must have access to the server snapshot")
        Assertions.assertEquals(0, inst.time())
        Assertions.assertEquals(0, inst.worldAge())
        Assertions.assertEquals(0, inst.chunks().size)
        Assertions.assertEquals(0, inst.entities().size)
    }
}