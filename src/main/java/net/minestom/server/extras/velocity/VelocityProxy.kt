...
/**
 * Support for [Velocity](https://velocitypowered.com/) modern forwarding.
 *
 *
 * Can be enabled by simply calling [.enable].
 */
object VelocityProxy {
    const val PLAYER_INFO_CHANNEL = "velocity:player_info"
     ...
     */
    fun enable(secret: String) {
        isEnabled = true
        key = SecretKeySpec(secret.toByteArray(), MAC_ALGORITHM)
    }

    @JvmStatic
    fun checkIntegrity(reader: BinaryReader): Boolean {
        val signature = reader.readBytes(32)
        val buf = reader.buffer
        buf.mark()
        val data = reader.readRemainingBytes()
        buf.reset()
        try {
    ...
    @JvmStatic
    fun readAddress(reader: BinaryReader): InetAddress? {
        return try {
            InetAddress.getByName(reader.readSizedString())
    ...
    @JvmStatic
    fun readSkin(reader: BinaryReader): PlayerSkin? {
        var skinTexture: String? = null
        var skinSignature: String? = null
        val properties = reader.readVarInt()
        for (i in 0 until properties) {
            val name = reader.readSizedString(Short.MAX_VALUE.toInt())
            val value = reader.readSizedString(Short.MAX_VALUE.toInt())
            val signature = if (reader.readBoolean()) reader.readSizedString(Short.MAX_VALUE.toInt()) else null
            if (name == "textures") {
                skinTexture = value
