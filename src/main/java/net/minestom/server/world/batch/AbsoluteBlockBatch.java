...
public class AbsoluteBlockBatch implements Batch<Runnable> {
    ...
    /**
     * Applies this batch to the given world.
     *
    ...
    public AbsoluteBlockBatch apply(@NotNull World world, @Nullable Runnable callback) {
        return apply(world, callback, true);
    }

    /**
     * Applies this batch to the given world, and execute the callback immediately when the
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    public AbsoluteBlockBatch unsafeApply(@NotNull World world, @Nullable Runnable callback) {
        return apply(world, callback, false);
    }

    /**
     * Applies this batch to the given world, and execute the callback depending on safeCallback.
     *
     * @param world        The World in which the batch should be applied
     * @param callback     The callback to be executed when the batch is applied
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    protected AbsoluteBlockBatch apply(@NotNull World world, @Nullable Runnable callback, boolean safeCallback) {
        ...
        synchronized (chunkBatchesMap) {
            ...
            for (Long2ObjectMap.Entry<ChunkBatch> entry : chunkBatchesMap.long2ObjectEntrySet()) {
                ...
                ChunkBatch chunkInverse = batch.apply(world, chunkX, chunkZ, c -> {
                    ...
                    if (isLast) {
                        ...
                        if (callback != null) {
                            if (safeCallback) {
                                world.scheduleNextTick(inst -> callback.run());
                            } else {
                                callback.run();
                            }
                        }
