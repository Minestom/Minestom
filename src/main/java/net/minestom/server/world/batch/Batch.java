...
public interface Batch<C> extends BlockSetter {
    ...
    /**
     * Gets if the batch is ready to be applied to a World.
     *
    ...
    /**
     * Called to apply the batch to the given World.
     * <p>
     * The implementation for all current batches executes the block updates in a dedicated pool,
     * and runs the callback on the next World update after block placement is complete. This
     ...
     */
    @Nullable
    Batch<C> apply(@NotNull World world, @Nullable C callback);
}
