...
public class RelativeBlockBatch implements Batch<Runnable> {
    ...
    /**
     * Applies this batch to the given World at the origin (0, 0, 0) of the World.
     *
     ...
     */
    @Override
    public AbsoluteBlockBatch apply(@NotNull World world, @Nullable Runnable callback) {
        return apply(world, 0, 0, 0, callback);
    }

    /**
     * Applies this batch to the given World at the given block position.
     *
     * @param world    The World in which the batch should be applied
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    public AbsoluteBlockBatch apply(@NotNull World world, @NotNull BlockPosition position, @Nullable Runnable callback) {
        return apply(world, position.getX(), position.getY(), position.getZ(), callback);
    }

    /**
     * Applies this batch to the given World at the given position.
     *
     * @param world    The World in which the batch should be applied
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    public AbsoluteBlockBatch apply(@NotNull World world, int x, int y, int z, @Nullable Runnable callback) {
        return apply(world, x, y, z, callback, true);
    }

    /**
     * Applies this batch to the given World at the given position, and execute the callback
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    public AbsoluteBlockBatch applyUnsafe(@NotNull World world, int x, int y, int z, @Nullable Runnable callback) {
        return apply(world, x, y, z, callback, false);
    }

    /**
     * Applies this batch to the given World at the given position, execute the callback depending on safeCallback.
     *
     * @param world        The World in which the batch should be applied
     * @param x            The x position to apply the batch
     * @param y            The y position to apply the batch
     * @param z            The z position to apply the batch
     * @param callback     The callback to be executed when the batch is applied
     * @param safeCallback If true, the callback will be executed in the next World update. Otherwise it will be executed immediately upon completion
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    protected AbsoluteBlockBatch apply(@NotNull World world, int x, int y, int z, @Nullable Runnable callback, boolean safeCallback) {
        return this.toAbsoluteBatch(x, y, z).apply(world, callback, safeCallback);
    }
