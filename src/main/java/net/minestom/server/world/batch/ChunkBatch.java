 ...
 * <p>
 * The batch can be placed in any chunk in any World, however it will always remain
...
public class ChunkBatch implements Batch<ChunkCallback> {
     ...
     *
     * @param world    The World in which the batch should be applied
     * @param callback The callback to be executed when the batch is applied
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    @Override
    public ChunkBatch apply(@NotNull World world, @Nullable ChunkCallback callback) {
        return apply(world, 0, 0, callback);
    }
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    public ChunkBatch apply(@NotNull World world, int chunkX, int chunkZ, @Nullable ChunkCallback callback) {
        final Chunk chunk = world.getChunk(chunkX, chunkZ);
        if (chunk == null) {
            LOGGER.warn("Unable to apply ChunkBatch to unloaded chunk ({}, {}) in {}.",
                    chunkX, chunkZ, world.getUniqueId());
            return null;
        }
        return apply(world, chunk, callback);
    }
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    public ChunkBatch apply(@NotNull World world, @NotNull Chunk chunk, @Nullable ChunkCallback callback) {
        return apply(world, chunk, callback, true);
    }
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    public ChunkBatch unsafeApply(@NotNull World world, @NotNull Chunk chunk, @Nullable ChunkCallback callback) {
        return apply(world, chunk, callback, false);
    }
     ...
     * @param chunk        The target chunk
     * @param callback     The callback to be executed when the batch is applied
     * @param safeCallback If true, the callback will be executed in the next world update.
     *                     Otherwise it will be executed immediately upon completion
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    protected ChunkBatch apply(@NotNull World world,
        ...
        BLOCK_BATCH_POOL.execute(() -> singleThreadFlush(world, chunk, inverse, callback, safeCallback));
        return inverse;
    }
     ...
     */
    private void singleThreadFlush(World world, Chunk chunk, @Nullable ChunkBatch inverse,
                                   @Nullable ChunkCallback callback, boolean safeCallback) {
        try {
            if (!chunk.isLoaded()) {
                LOGGER.warn("Unable to apply ChunkBatch to unloaded chunk ({}, {}) in {}.",
                        chunk.getChunkX(), chunk.getChunkZ(), world.getUniqueId());
                return;
            }
            ...
            updateChunk(world, chunk, sections, callback, safeCallback);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     */
    private void updateChunk(@NotNull World world, Chunk chunk, IntSet updatedSections, @Nullable ChunkCallback callback, boolean safeCallback) {
        ...
        if (world instanceof WorldContainer) {
            // FIXME: put method in World instead
            ((WorldContainer) world).refreshLastBlockChangeTime();
        }

        if (callback != null) {
            if (safeCallback) {
                world.scheduleNextTick(inst -> callback.accept(chunk));
            } else {
