...
/**
 * A chunk is a part of an {@link World}, limited by a size of 16x256x16 blocks and subdivided in 16 sections of 16 blocks height.
 ...
 * and create the objects in {@link WorldContainer#setChunkSupplier(ChunkSupplier)}.
 * <p>
...
public abstract class Chunk implements BlockGetter, BlockSetter, Viewable, Tickable, DataContainer {
    ...
    public Chunk(@NotNull World world, @Nullable Biome[] biomes, int chunkX, int chunkZ, boolean shouldGenerate) {
        ...
        final int biomeCount = Biome.getBiomeCount(world.getDimensionType());
     ...
     * <p>
     * WARNING: this method is not thread-safe (in order to bring performance improvement with {@link net.minestom.server.world.batch.Batch batches})
     * The thread-safe version is {@link World#setBlock(int, int, int, Block)} (or any similar World methods)
     ...
     *                 WARNING: this need to be called to notify the World.
     ...
     *
     * @param world    the chunk owner
     * @param chunkX   the chunk X of the copy
     * @param chunkZ   the chunk Z of the copy
     * @return a copy of this chunk with a potentially new World and position
     */
    @NotNull
    public abstract Chunk copy(@NotNull World world, int chunkX, int chunkZ);
    ...
    /**
     * Gets the World where this chunk is stored
     *
     * @return the linked World
     */
    @NotNull
    ...
    public UpdateLightPacket getLightPacket() {
        ...
        final int sectionCount = (getWorld().getDimensionType().getTotalHeight() / 16) + 2;
