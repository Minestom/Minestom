...
/**
 * A {@link IChunkLoader} used by {@link WorldContainer}
...
public class MinestomBasicChunkLoader implements IChunkLoader {
     ...
     *
     * @param worldContainer the {@link WorldContainer} linked to this loader
     */
    public MinestomBasicChunkLoader(WorldContainer worldContainer) {
        this.worldContainer = worldContainer;
    }

    @Override
    public void saveChunk(@NotNull Chunk chunk, @Nullable Runnable callback) {
        final StorageLocation storageLocation = worldContainer.getStorageLocation();
    ...
    @Override
    public boolean loadChunk(@NotNull World world, int chunkX, int chunkZ, @Nullable ChunkCallback callback) {
        final StorageLocation storageLocation = worldContainer.getStorageLocation();
        ...
        if (bytes == null) {
            ...
            // Create the chunk object using the World's ChunkSupplier to support multiple implementations
            Chunk chunk = worldContainer.getChunkSupplier().createChunk(world, null, chunkX, chunkZ);
