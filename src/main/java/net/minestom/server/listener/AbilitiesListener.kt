...
object AbilitiesListener {
    fun listener(packet: ClientPlayerAbilitiesPacket, player: Player) {
        val canFly = player.isAllowFlying || player.isCreative
        if (canFly) {
            val isFlying: Boolean = packet.flags() and 0x2 > 0
            player.refreshFlying(isFlying)
            if (isFlying) {
                val startFlyingEvent = PlayerStartFlyingEvent(player)
                EventDispatcher.call(startFlyingEvent)
            } else {
                val stopFlyingEvent = PlayerStopFlyingEvent(player)
