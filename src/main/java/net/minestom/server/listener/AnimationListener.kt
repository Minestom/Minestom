...
object AnimationListener {
    fun animationListener(packet: ClientAnimationPacket, player: Player) {
        val hand = packet.hand()
        val itemStack = player.getItemInHand(hand)
        //itemStack.onLeftClick(player, hand);
        val handAnimationEvent = PlayerHandAnimationEvent(player, hand)
        EventDispatcher.callCancellable(handAnimationEvent) {
            when (hand) {
                Hand.MAIN -> player.swingMainHand()
                Hand.OFF -> player.swingOffHand()
            }
        }
    }
}