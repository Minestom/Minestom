...
public final class EntityUtils {
    ...
    public static Point getPassengerPositionOffset(Entity vehicle, Entity passenger, int passengerIndex) {
        ...
        if (vehicle.getEntityType() == EntityType.CAMEL || vehicle.getEntityType() == EntityType.CAMEL_HUSK) {
            if (passengerIndex == 0) {
                return new Vec(0, vehicle.getEntityType().height(), 0.5).rotateAroundY(Math.toRadians(vehicle.getPosition().yaw() * -1));
            } else {
                return new Vec(0, vehicle.getEntityType().height(), -0.7).rotateAroundY(Math.toRadians(vehicle.getPosition().yaw() * -1));
            }
        } else if (vehicle.getEntityType().name().contains("boat")) {
            if (passengerIndex == 0) {
                return new Vec(0, vehicle.getEntityType().height() / 3f, 0.2).rotateAroundY(Math.toRadians(vehicle.getPosition().yaw() * -1));
            } else {
                return new Vec(0, vehicle.getEntityType().height() / 3f, -0.6).rotateAroundY(Math.toRadians(vehicle.getPosition().yaw() * -1));
            }
