 ...
 */
class ArgumentString(id: String) : Argument<String>(id, true) {
    ...
    override fun toString(): String {
        return String.format("String<%s>", id)
    }
    ...
    companion object {
        ...
        const val QUOTE_ERROR = 1

        @Deprecated("use {@link Argument#parse(Argument)}")
        @Throws(ArgumentSyntaxException::class)
        fun staticParse(input: String): String {
            // Return if not quoted
            var input = input
            if (!input.contains(DOUBLE_QUOTE.toString()) &&
            ...
            val quote = input.length >= 2 && first == last && (first == DOUBLE_QUOTE || first == QUOTE)
            if (!quote) throw ArgumentSyntaxException(
                "String argument needs to start and end with quotes",
                input,
                QUOTE_ERROR
            )
            ...
            for (i in 1 until input.length) {
                val c = input[i]
                if (c == first) {
                    val lastChar = input[i - 1]
                    if (lastChar != BACKSLASH) {
                        throw ArgumentSyntaxException("Non-escaped quote", input, QUOTE_ERROR)
                    }
                }
            }
