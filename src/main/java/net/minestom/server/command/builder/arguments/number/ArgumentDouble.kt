...
class ArgumentDouble(id: String) : ArgumentNumber<Double?>(
    id,
    "brigadier:double",
    Function { s: String -> s.toDouble() },
    BiFunction { s: String, radix: Int? ->
        s.toLong(
            radix!!
        ).toDouble()
    },
    BiConsumer { obj: BinaryWriter, d: Double? -> obj.writeDouble(d!!) },
    java.util.Comparator { d1: Double?, d2: Double? ->
        java.lang.Double.compare(
            d1!!, d2!!
        )
    }) {
    override fun toString(): String {
        return String.format("Double<%s>", id)
    }
}