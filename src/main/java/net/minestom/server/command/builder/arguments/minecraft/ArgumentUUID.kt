...
class ArgumentUUID(id: String) : Argument<UUID>(id) {
    @Throws(ArgumentSyntaxException::class)
    override fun parse(input: String): UUID {
        return try {
            UUID.fromString(input)
        } catch (exception: IllegalArgumentException) {
            throw ArgumentSyntaxException("Invalid UUID", input, INVALID_UUID)
        }
    }
    ...
    override fun toString(): String {
        return String.format("UUID<%s>", id)
    }

    companion object {
        const val INVALID_UUID = -1
    }
}