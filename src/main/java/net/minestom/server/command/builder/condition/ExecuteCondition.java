...
@FunctionalInterface
public interface ExecuteCondition extends Condition {
    /**
    ...
    static boolean canExecute(@NotNull Collection<ExecuteCondition> conditions, @NotNull CommandSender sender, @Nullable String command) {
        for (ExecuteCondition condition : conditions) {
            if (!condition.canUse(sender, command)) {
                return false;
            }
        }
        return true;
    ...
    default Collection<Player> getPlayers() {
        return MinecraftServer.getConnectionManager().getOnlinePlayers().stream().filter(x -> canUse(x, null)).collect(Collectors.toList());
    }
}
