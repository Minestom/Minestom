...
abstract class ArgumentRegistry<T>(id: String) : Argument<T>(id) {
    abstract fun getRegistry(value: String): T

    @Throws(ArgumentSyntaxException::class)
    override fun parse(input: String): T {
        return getRegistry(input)
            ?: throw ArgumentSyntaxException("Registry value is invalid", input, INVALID_NAME)
    }
