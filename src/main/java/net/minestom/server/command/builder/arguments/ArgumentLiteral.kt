...
class ArgumentLiteral(id: String) : Argument<String>(id) {
    @Throws(ArgumentSyntaxException::class)
    override fun parse(input: String): String {
        if (input != id) throw ArgumentSyntaxException("Invalid literal value", input, INVALID_VALUE_ERROR)
        return input
    }

    override fun processNodes(nodeMaker: NodeMaker, executable: Boolean) {
        val literalNode = DeclareCommandsPacket.Node()
        literalNode.flags = DeclareCommandsPacket.getFlag(
            DeclareCommandsPacket.NodeType.LITERAL,
            executable, false, false
        )
        literalNode.name = id
        nodeMaker.addNodes(arrayOf(literalNode))
    }

    override fun toString(): String {
        return String.format("Literal<%s>", id)
    }

    companion object {
        const val INVALID_VALUE_ERROR = 1
    }
}