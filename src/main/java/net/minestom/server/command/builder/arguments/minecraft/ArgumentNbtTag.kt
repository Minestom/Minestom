...
/**
 * Argument used to retrieve a [NBT] based object, can be any kind of tag like
 * [org.jglrxavpok.hephaistos.nbt.NBTCompound], [org.jglrxavpok.hephaistos.nbt.NBTList],
 * [org.jglrxavpok.hephaistos.nbt.NBTInt], etc...
 *
 *
 * Example: {display:{Name:"{\"text\":\"Sword of Power\"}"}} or [{display:{Name:"{\"text\":\"Sword of Power\"}"}}]
 */
class ArgumentNbtTag(id: String) : Argument<NBT>(id, true) {
    @Throws(ArgumentSyntaxException::class)
    override fun parse(input: String): NBT {
        return try {
            SNBTParser(StringReader(input)).parse()
        } catch (e: NBTException) {
            throw ArgumentSyntaxException("Invalid NBT", input, INVALID_NBT)
        }
    }
    ...
    override fun toString(): String {
        return String.format("NBT<%s>", id)
    }

    companion object {
        const val INVALID_NBT = 1
    }
}