 ...
 */
class ArgumentNbtCompoundTag(id: String) : Argument<NBTCompound>(id, true) {
    @Throws(ArgumentSyntaxException::class)
    override fun parse(input: String): NBTCompound {
        return try {
            val nbt = SNBTParser(StringReader(input)).parse() as? NBTCompound
                ?: throw ArgumentSyntaxException("NBTCompound is invalid", input, INVALID_NBT)
            nbt
    ...
    override fun toString(): String {
        return String.format("NbtCompound<%s>", id)
    }
