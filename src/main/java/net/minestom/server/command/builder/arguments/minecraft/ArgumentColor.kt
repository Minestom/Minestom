...
/**
 * Represents an argument which will give you a [Style] containing the colour or no
 * colour if the argument was `reset`.
 *
 *
 * Example: red, white, reset
 */
class ArgumentColor(id: String) : Argument<Style>(id) {
    @Throws(ArgumentSyntaxException::class)
    override fun parse(input: String): Style {
        ...
        throw ArgumentSyntaxException("Undefined color", input, UNDEFINED_COLOR)
    }
    ...
    override fun toString(): String {
        return String.format("Color<%s>", id)
    }

    companion object {
        const val UNDEFINED_COLOR = -2
    }
}