...
class ArgumentComponent(id: String) : Argument<Component>(id, true) {
    @Throws(ArgumentSyntaxException::class)
    override fun parse(input: String): Component {
        return try {
            GsonComponentSerializer.gson().deserialize(input)
        } catch (e: JsonParseException) {
            throw ArgumentSyntaxException("Invalid JSON", input, INVALID_JSON_ERROR)
        }
    }
    ...
    override fun toString(): String {
        return String.format("Component<%s>", id)
    }

    companion object {
        const val INVALID_JSON_ERROR = 1
    }
}