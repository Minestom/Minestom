...
class WitherMeta(entity: Entity, metadata: Metadata) : MonsterMeta(entity, metadata) {
    private var centerHead: Entity? = null
    private var leftHead: Entity? = null
    private var rightHead: Entity? = null
    fun getCenterHead(): Entity? {
        return centerHead
    }

    fun setCenterHead(value: Entity?) {
        centerHead = value
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(value?.entityId ?: 0))
    }

    fun getLeftHead(): Entity? {
        return leftHead
    }

    fun setLeftHead(value: Entity?) {
        leftHead = value
        super.metadata.setIndex(OFFSET + 1, Metadata.VarInt(value?.entityId ?: 0))
    }

    fun getRightHead(): Entity? {
        return rightHead
    }

    fun setRightHead(value: Entity?) {
        rightHead = value
        super.metadata.setIndex(OFFSET + 2, Metadata.VarInt(value?.entityId ?: 0))
    }
    ...
    companion object {
        val OFFSET: Byte = MonsterMeta.Companion.MAX_OFFSET
