...
open class MobMeta protected constructor(entity: Entity, metadata: Metadata) : LivingEntityMeta(entity, metadata) {
    var isNoAi: Boolean
        get() = getMaskBit(OFFSET.toInt(), NO_AI_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), NO_AI_BIT, value)
        }
    var isLeftHanded: Boolean
        get() = getMaskBit(OFFSET.toInt(), IS_LEFT_HANDED_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), IS_LEFT_HANDED_BIT, value)
        }
    var isAggressive: Boolean
        get() = getMaskBit(OFFSET.toInt(), IS_AGGRESSIVE_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), IS_AGGRESSIVE_BIT, value)
        }

    companion object {
        val OFFSET: Byte = LivingEntityMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 1).toByte()
        private const val NO_AI_BIT: Byte = 0x01
        private const val IS_LEFT_HANDED_BIT: Byte = 0x02
        private const val IS_AGGRESSIVE_BIT: Byte = 0x04
    }
}