...
class IronGolemMeta(entity: Entity, metadata: Metadata) : AbstractGolemMeta(entity, metadata) {
    var isPlayerCreated: Boolean
        get() = getMaskBit(OFFSET.toInt(), PLAYER_CREATED_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), PLAYER_CREATED_BIT, value)
        }

    companion object {
        val OFFSET: Byte = AbstractGolemMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 1).toByte()
        private const val PLAYER_CREATED_BIT: Byte = 0x01
    }
}