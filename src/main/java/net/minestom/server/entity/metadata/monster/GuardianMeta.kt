...
open class GuardianMeta(entity: Entity, metadata: Metadata) : MonsterMeta(entity, metadata) {
    private var target: Entity? = null
    fun isRetractingSpikes(): Boolean {
        return super.metadata.getIndex(OFFSET.toInt(), false)
    }

    fun setRetractingSpikes(retractingSpikes: Boolean) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.Boolean(retractingSpikes))
    }
    ...
    fun setTarget(target: Entity?) {
        this.target = target
        super.metadata.setIndex(OFFSET + 1, Metadata.VarInt(target?.entityId ?: 0))
    }
