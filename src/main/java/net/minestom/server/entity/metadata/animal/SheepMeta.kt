...
class SheepMeta(entity: Entity, metadata: Metadata) : AnimalMeta(entity, metadata) {
    fun getColor(): Int {
        return getMask(OFFSET.toInt()) and COLOR_BITS
    }

    fun setColor(color: Byte) {
        val before = getMask(OFFSET.toInt())
        var mask = before
        mask = mask and COLOR_BITS.inv()
        mask = mask or (color and COLOR_BITS)
        if (mask != before) {
            setMask(OFFSET.toInt(), mask)
        }
    }

    fun isSheared(): Boolean {
        return getMaskBit(OFFSET.toInt(), SHEARED_BIT)
    }

    fun setSheared(value: Boolean) {
        setMaskBit(OFFSET.toInt(), SHEARED_BIT, value)
    }

    companion object {
        val OFFSET: Byte = AnimalMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 1).toByte()
        private const val COLOR_BITS: Byte = 0x0F
        private const val SHEARED_BIT: Byte = 0x10
    }
}