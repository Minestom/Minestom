...
class CreeperMeta(entity: Entity, metadata: Metadata) : MonsterMeta(entity, metadata) {
    fun getState(): State {
        val id = super.metadata.getIndex(OFFSET.toInt(), -1)
        return if (id == -1) State.IDLE else State.FUSE
    }

    fun setState(value: State) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(if (value == State.IDLE) -1 else 1))
    }
