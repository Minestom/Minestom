...
class EnderDragonMeta(entity: Entity, metadata: Metadata) : MobMeta(entity, metadata) {
    fun getPhase(): Phase {
        return Phase.VALUES[super.metadata.getIndex(OFFSET.toInt(), 0)]
    }

    fun setPhase(value: Phase) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(value.ordinal))
    }

    enum class Phase {
        CIRCLING, STRAFING, FLYING_TO_THE_PORTAL, LANDING_ON_THE_PORTAL, TAKING_OFF_FROM_THE_PORTAL, BREATH_ATTACK, LOOKING_FOR_BREATH_ATTACK_PLAYER, ROAR, CHARGING_PLAYER, FLYING_TO_THE_PORTAL_TO_DIE, HOVERING_WITHOUT_AI;
