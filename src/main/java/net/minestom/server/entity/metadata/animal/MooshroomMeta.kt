...
class MooshroomMeta(entity: Entity, metadata: Metadata) : CowMeta(entity, metadata) {
    fun getVariant(): Variant {
        return Variant.valueOf(super.metadata.getIndex(OFFSET.toInt(), "red").toUpperCase(Locale.ROOT))
    }

    fun setVariant(value: Variant) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.String(value.name.toLowerCase(Locale.ROOT)))
    }
    ...
    companion object {
        val OFFSET: Byte = CowMeta.Companion.MAX_OFFSET
