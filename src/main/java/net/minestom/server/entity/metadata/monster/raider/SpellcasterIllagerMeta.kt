...
open class SpellcasterIllagerMeta protected constructor(entity: Entity, metadata: Metadata) :
    AbstractIllagerMeta(entity, metadata) {
    fun getSpell(): Spell {
        return Spell.VALUES[super.metadata.getIndex(OFFSET.toInt(), 0.toByte()).toInt()]
    }

    fun setSpell(spell: Spell) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.Byte(spell.ordinal.toByte()))
    }

    enum class Spell {
        NONE, SUMMON_VEX, ATTACK, WOLOLO, DISAPPEAR, BLINDNESS;
    ...
    companion object {
        val OFFSET: Byte = AbstractIllagerMeta.Companion.MAX_OFFSET
