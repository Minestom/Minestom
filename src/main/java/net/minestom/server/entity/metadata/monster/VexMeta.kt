...
class VexMeta(entity: Entity, metadata: Metadata) : MonsterMeta(entity, metadata) {
    fun isAttacking(): Boolean {
        return getMaskBit(OFFSET.toInt(), ATTACKING_BIT)
    }

    fun setAttacking(value: Boolean) {
        setMaskBit(OFFSET.toInt(), ATTACKING_BIT, value)
    }

    companion object {
        val OFFSET: Byte = MonsterMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 1).toByte()
        private const val ATTACKING_BIT: Byte = 0x01
    }
}