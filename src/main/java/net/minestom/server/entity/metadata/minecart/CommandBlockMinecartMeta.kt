...
class CommandBlockMinecartMeta(entity: Entity, metadata: Metadata) : AbstractMinecartMeta(entity, metadata) {
    fun getCommand(): String {
        return super.metadata.getIndex(OFFSET.toInt(), "")
    }

    fun setCommand(value: String) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.String(value))
    }

    fun getLastOutput(): Component {
        return super.metadata.getIndex(OFFSET + 1, Component.empty())
    }

    fun setLastOutput(value: Component) {
    ...
    companion object {
        val OFFSET: Byte = AbstractMinecartMeta.Companion.MAX_OFFSET
