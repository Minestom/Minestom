...
class GlowSquidMeta(entity: Entity, metadata: Metadata) : WaterAnimalMeta(entity, metadata) {
    private fun getDarkTicksRemaining(): Int {
        return metadata.getIndex(OFFSET.toInt(), 0)
    }

    private fun setDarkTicksRemaining(ticks: Int) {
        metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(ticks))
    }
