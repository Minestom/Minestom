...
class ArmorStandMeta(entity: Entity, metadata: Metadata) : LivingEntityMeta(entity, metadata) {
    fun isSmall(): Boolean {
        return getMaskBit(OFFSET.toInt(), IS_SMALL_BIT)
    }

    fun setSmall(value: Boolean) {
        setMaskBit(OFFSET.toInt(), IS_SMALL_BIT, value)
    }

    fun isHasArms(): Boolean {
        return getMaskBit(OFFSET.toInt(), HAS_ARMS_BIT)
    }

    fun setHasArms(value: Boolean) {
        setMaskBit(OFFSET.toInt(), HAS_ARMS_BIT, value)
    }

    fun isHasNoBasePlate(): Boolean {
        return getMaskBit(OFFSET.toInt(), HAS_NO_BASE_PLATE_BIT)
    }

    fun setHasNoBasePlate(value: Boolean) {
        setMaskBit(OFFSET.toInt(), HAS_NO_BASE_PLATE_BIT, value)
    }

    fun isMarker(): Boolean {
        return getMaskBit(OFFSET.toInt(), IS_MARKER_BIT)
    }

    fun setMarker(value: Boolean) {
        setMaskBit(OFFSET.toInt(), IS_MARKER_BIT, value)
    }

    fun getHeadRotation(): Vec {
        return super.metadata.getIndex(OFFSET + 1, Vec.ZERO)
    }

    fun setHeadRotation(value: Vec) {
        super.metadata.setIndex(OFFSET + 1, Metadata.Rotation(value))
    }

    fun getBodyRotation(): Vec {
        return super.metadata.getIndex(OFFSET + 2, Vec.ZERO)
    }

    fun setBodyRotation(value: Vec) {
        super.metadata.setIndex(OFFSET + 2, Metadata.Rotation(value))
    }

    fun getLeftArmRotation(): Vec {
        return super.metadata.getIndex(OFFSET + 3, Vec(-10.0, 0.0, -10.0))
    }

    fun setLeftArmRotation(value: Vec) {
        super.metadata.setIndex(OFFSET + 3, Metadata.Rotation(value))
    }

    fun getRightArmRotation(): Vec {
        return super.metadata.getIndex(OFFSET + 4, Vec(-15.0, 0.0, 10.0))
    }

    fun setRightArmRotation(value: Vec) {
        super.metadata.setIndex(OFFSET + 4, Metadata.Rotation(value))
    }

    fun getLeftLegRotation(): Vec {
        return super.metadata.getIndex(OFFSET + 5, Vec(-1.0, 0.0, -1.0))
    }

    fun setLeftLegRotation(value: Vec) {
        super.metadata.setIndex(OFFSET + 5, Metadata.Rotation(value))
    }

    fun getRightLegRotation(): Vec {
        return super.metadata.getIndex(OFFSET + 6, Vec(1.0, 0.0, 1.0))
    }

    fun setRightLegRotation(value: Vec) {
        super.metadata.setIndex(OFFSET + 6, Metadata.Rotation(value))
    }

    companion object {
        val OFFSET: Byte = LivingEntityMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 7).toByte()
        private const val IS_SMALL_BIT: Byte = 0x01
        private const val HAS_ARMS_BIT: Byte = 0x04
        private const val HAS_NO_BASE_PLATE_BIT: Byte = 0x08
        private const val IS_MARKER_BIT: Byte = 0x10
    }
}