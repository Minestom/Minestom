...
open class AbstractArrowMeta protected constructor(entity: Entity, metadata: Metadata) : EntityMeta(entity, metadata) {
    var isCritical: Boolean
        get() = getMaskBit(OFFSET.toInt(), CRITICAL_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), CRITICAL_BIT, value)
        }
    var isNoClip: Boolean
        get() = getMaskBit(OFFSET.toInt(), NO_CLIP_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), NO_CLIP_BIT, value)
        }
    var piercingLevel: Byte
        get() = super.metadata.getIndex(OFFSET + 1, 0.toByte())
        set(value) {
    ...
    companion object {
        ...
        private const val CRITICAL_BIT: Byte = 0x01
        private const val NO_CLIP_BIT: Byte = 0x01
    }
}