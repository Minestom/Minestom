...
class FoxMeta(entity: Entity, metadata: Metadata) : AnimalMeta(entity, metadata) {
    fun getType(): Type {
        return Type.VALUES[super.metadata.getIndex(
            OFFSET.toInt(),
            0
        )]
    }
    ...
    fun setType(type: Type) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(type.ordinal))
    }
    ...
    fun isFoxSneaking(): Boolean {
        return getMaskBit(OFFSET + 1, CROUCHING_BIT)
    }

    fun setFoxSneaking(value: Boolean) {
    ...
    fun getFirstUUID(): UUID? {
        return super.metadata.getIndex<UUID>(OFFSET + 2, null)
    }

    fun setFirstUUID(value: UUID?) {
        super.metadata.setIndex(OFFSET + 2, Metadata.OptUUID(value))
    }

    fun getSecondUUID(): UUID? {
        return super.metadata.getIndex<UUID>(OFFSET + 3, null)
    }

    fun setSecondUUID(value: UUID?) {
    ...
    companion object {
        ...
        val MAX_OFFSET = (OFFSET + 4).toByte()
        private const val SITTING_BIT: Byte = 0x01
        private const val CROUCHING_BIT: Byte = 0x04
        private const val INTERESTED_BIT: Byte = 0x08
        private const val POUNCING_BIT: Byte = 0x10
        private const val SLEEPING_BIT: Byte = 0x20
        private const val FACEPLANTED_BIT: Byte = 0x40
        private const val DEFENDING_BIT = 0x80.toByte()
    }
}