...
open class SpiderMeta(entity: Entity, metadata: Metadata) : MonsterMeta(entity, metadata) {
    fun isClimbing(): Boolean {
        return getMaskBit(OFFSET.toInt(), CLIMBING_BIT)
    }

    fun setClimbing(value: Boolean) {
        setMaskBit(OFFSET.toInt(), CLIMBING_BIT, value)
    }

    companion object {
        val OFFSET: Byte = MonsterMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 1).toByte()
        private const val CLIMBING_BIT: Byte = 0x01
    }
}