...
class ShulkerMeta(entity: Entity, metadata: Metadata) : AbstractGolemMeta(entity, metadata) {
    var attachFace: Direction?
        get() = super.metadata.getIndex(OFFSET.toInt(), Direction.DOWN)
        set(value) {
            super.metadata.setIndex(
                OFFSET.toInt(), Metadata.Direction(
                    value!!
                )
            )
        }
    var attachmentPosition: Point?
        get() = super.metadata.getIndex<Point>(OFFSET + 1, null)
        set(value) {
            super.metadata.setIndex(OFFSET + 1, Metadata.OptPosition(value))
        }
    var shieldHeight: Byte
        get() = super.metadata.getIndex(OFFSET + 2, 0.toByte())
        set(value) {
            super.metadata.setIndex(OFFSET + 2, Metadata.Byte(value))
        }
    var color: Byte
        get() = super.metadata.getIndex(OFFSET + 3, 10.toByte())
        set(value) {
            super.metadata.setIndex(OFFSET + 3, Metadata.Byte(value))
        }

    companion object {
        val OFFSET: Byte = AbstractGolemMeta.Companion.MAX_OFFSET
