...
class ZombieVillagerMeta(entity: Entity, metadata: Metadata) : ZombieMeta(entity, metadata) {
    fun isConverting(): Boolean {
        return super.metadata.getIndex(OFFSET.toInt(), false)
    }

    fun setConverting(value: Boolean) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.Boolean(value))
    }

    fun getVillagerData(): VillagerData {
        val data = super.metadata.getIndex<IntArray>(OFFSET + 1, null)
            ?: return VillagerData(
        ...
        return VillagerData(
            VillagerMeta.Type.Companion.VALUES.get(data[0]), Profession.Companion.VALUES.get(
                data[1]
            ), VillagerMeta.Level.Companion.VALUES.get(data[2] - 1)
        )
    }

    fun setVillagerData(data: VillagerData) {
        super.metadata.setIndex(
            OFFSET + 1, Metadata.VillagerData(
                data.type.ordinal,
    ...
    companion object {
        val OFFSET: Byte = ZombieMeta.Companion.MAX_OFFSET
