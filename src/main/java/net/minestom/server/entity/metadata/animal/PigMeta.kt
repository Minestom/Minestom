...
class PigMeta(entity: Entity, metadata: Metadata) : AnimalMeta(entity, metadata) {
    fun isHasSaddle(): Boolean {
        return super.metadata.getIndex(OFFSET.toInt(), false)
    }

    fun setHasSaddle(value: Boolean) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.Boolean(value))
    }

    fun getTimeToBoost(): Int {
        return super.metadata.getIndex(OFFSET + 1, 0)
    }

    fun setTimeToBoost(value: Int) {
