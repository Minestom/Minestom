...
class FishingHookMeta(entity: Entity, metadata: Metadata) : EntityMeta(entity, metadata), ObjectDataProvider {
    private var hooked: Entity? = null
    private var owner: Entity? = null
    fun getHookedEntity(): Entity? {
        return hooked
    }

    fun setHookedEntity(value: Entity?) {
        hooked = value
        val entityID = if (value == null) 0 else value.entityId + 1
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(entityID))
    }

    fun isCatchable(): Boolean {
        return super.metadata.getIndex(OFFSET + 1, false)
    }

    fun setCatchable(value: Boolean) {
        super.metadata.setIndex(OFFSET + 1, Metadata.Boolean(value))
    }

    fun getOwnerEntity(): Entity? {
        return owner
    }

    fun setOwnerEntity(value: Entity?) {
        owner = value
    }

    override fun getObjectData(): Int {
        return if (owner != null) owner.getEntityId() else 0
    }
