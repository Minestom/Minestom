...
class PufferfishMeta(entity: Entity, metadata: Metadata) : AbstractFishMeta(entity, metadata) {
    init {
        updateBoundingBox(State.UNPUFFED)
    }

    fun getState(): State {
        return State.VALUES[super.metadata.getIndex(
            OFFSET.toInt(),
            0
        )]
    }
    ...
    fun setState(state: State) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(state.ordinal))
        updateBoundingBox(state)
    }

    private fun updateBoundingBox(state: State) {
        consumeEntity { entity: Entity? ->
            when (state) {
                State.UNPUFFED -> entity.setBoundingBox(.35, .35, .35)
                State.SEMI_PUFFED -> entity.setBoundingBox(.5, .5, .5)
                else -> entity.setBoundingBox(.7, .7, .7)
            }
        }
    }

    enum class State {
        UNPUFFED, SEMI_PUFFED, FULLY_PUFFED;
