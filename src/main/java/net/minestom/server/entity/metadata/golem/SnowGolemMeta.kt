...
class SnowGolemMeta(entity: Entity, metadata: Metadata) : AbstractGolemMeta(entity, metadata) {
    var isHasPumpkinHat: Boolean
        get() = super.metadata.getIndex(OFFSET.toInt(), 0x10.toByte()) == 0x10.toByte()
        set(value) {
            super.metadata.setIndex(OFFSET.toInt(), Metadata.Byte(if (value) 0x10.toByte() else 0x00.toByte()))
        }
