...
open class EntityMeta(entity: Entity?, metadata: Metadata) {
     ...
     * [net.minestom.server.entity.LivingEntity.refreshActiveHand].
     *
     * @param notifyAboutChanges if to notify entity viewers about this meta changes.
     */
    fun setNotifyAboutChanges(notifyAboutChanges: Boolean) {
        metadata.setNotifyAboutChanges(notifyAboutChanges)
    }

    open var isOnFire: Boolean
        get() = getMaskBit(OFFSET.toInt(), ON_FIRE_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), ON_FIRE_BIT, value)
        }
    var isSneaking: Boolean
        get() = getMaskBit(OFFSET.toInt(), CROUCHING_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), CROUCHING_BIT, value)
        }
    var isSprinting: Boolean
        get() = getMaskBit(OFFSET.toInt(), SPRINTING_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), SPRINTING_BIT, value)
        }
    var isSwimming: Boolean
        get() = getMaskBit(OFFSET.toInt(), SWIMMING_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), SWIMMING_BIT, value)
        }
    var isInvisible: Boolean
        get() = getMaskBit(OFFSET.toInt(), INVISIBLE_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), INVISIBLE_BIT, value)
        }
    var isHasGlowingEffect: Boolean
        get() = getMaskBit(OFFSET.toInt(), HAS_GLOWING_EFFECT_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), HAS_GLOWING_EFFECT_BIT, value)
        }
    var isFlyingWithElytra: Boolean
        get() = getMaskBit(OFFSET.toInt(), FLYING_WITH_ELYTRA_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), FLYING_WITH_ELYTRA_BIT, value)
        }
    var airTicks: Int
        get() = metadata.getIndex(OFFSET + 1, 300)
        set(value) {
            metadata.setIndex(OFFSET + 1, Metadata.VarInt(value))
        }
    var customName: Component?
        get() = metadata.getIndex<Component>(OFFSET + 2, null)
        set(value) {
            metadata.setIndex(OFFSET + 2, Metadata.OptChat(value))
        }
    var isCustomNameVisible: Boolean
        get() = metadata.getIndex(OFFSET + 3, false)
        set(value) {
            metadata.setIndex(OFFSET + 3, Metadata.Boolean(value))
        }
    var isSilent: Boolean
        get() = metadata.getIndex(OFFSET + 4, false)
        set(value) {
            metadata.setIndex(OFFSET + 4, Metadata.Boolean(value))
        }
    var isHasNoGravity: Boolean
        get() = metadata.getIndex(OFFSET + 5, false)
        set(value) {
            metadata.setIndex(OFFSET + 5, Metadata.Boolean(value))
        }
    var pose: Pose?
        get() = metadata.getIndex(OFFSET + 6, Pose.STANDING)
        set(value) {
            metadata.setIndex(OFFSET + 6, Metadata.Pose(value))
        }
    var tickFrozen: Int
        get() = metadata.getIndex(OFFSET + 7, 0)
        set(tickFrozen) {
            metadata.setIndex(OFFSET + 7, Metadata.VarInt(tickFrozen))
        }

    protected fun getMask(index: Int): Byte {
        return metadata.getIndex(index, 0.toByte())
    }

    protected fun setMask(index: Int, mask: Byte) {
        metadata.setIndex(index, Metadata.Byte(mask))
    }

    protected fun getMaskBit(index: Int, bit: Byte): Boolean {
        return getMask(index) and bit == bit.toInt()
    }

    protected fun setMaskBit(index: Int, bit: Byte, value: Boolean) {
        var mask = getMask(index)
        val currentValue = mask and bit == bit.toInt()
    ...
    protected fun consumeEntity(consumer: Consumer<Entity?>) {
    ...
    companion object {
        const val OFFSET: Byte = 0
        const val MAX_OFFSET = (OFFSET + 8).toByte()
        private const val ON_FIRE_BIT: Byte = 0x01
        private const val CROUCHING_BIT: Byte = 0x02
        private const val SPRINTING_BIT: Byte = 0x08
        private const val SWIMMING_BIT: Byte = 0x10
        private const val INVISIBLE_BIT: Byte = 0x20
        private const val HAS_GLOWING_EFFECT_BIT: Byte = 0x40
        private const val FLYING_WITH_ELYTRA_BIT = 0x80.toByte()
    }
}