...
abstract class AbstractMinecartMeta protected constructor(entity: Entity, metadata: Metadata) :
    EntityMeta(entity, metadata), ObjectDataProvider {
    fun getShakingPower(): Int {
        return super.metadata.getIndex(OFFSET.toInt(), 0)
    }

    fun setShakingPower(value: Int) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(value))
    }

    fun getShakingDirection(): Int {
        return super.metadata.getIndex(OFFSET + 1, 1)
    }

    fun setShakingDirection(value: Int) {
        super.metadata.setIndex(OFFSET + 1, Metadata.VarInt(value))
    }

    fun getShakingMultiplier(): Float {
        return super.metadata.getIndex(OFFSET + 2, 0f)
    }

    fun setShakingMultiplier(value: Float) {
        super.metadata.setIndex(OFFSET + 2, Metadata.Float(value))
    }

    fun getCustomBlockIdAndDamage(): Int {
        return super.metadata.getIndex(OFFSET + 3, 0)
    }

    fun setCustomBlockIdAndDamage(value: Int) {
        super.metadata.setIndex(OFFSET + 3, Metadata.VarInt(value))
    }

    // in 16th of a block
    fun getCustomBlockYPosition(): Int {
        return super.metadata.getIndex(OFFSET + 4, 6)
    }

    fun setCustomBlockYPosition(value: Int) {
