...
class PlayerMeta(entity: Entity, metadata: Metadata) : LivingEntityMeta(entity, metadata) {
    fun getAdditionalHearts(): Float {
        return super.metadata.getIndex(OFFSET.toInt(), 0f)
    }

    fun setAdditionalHearts(value: Float) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.Float(value))
    }
    ...
    fun setHatEnabled(value: Boolean) {
        setMaskBit(OFFSET + 2, HAT_BIT, value)
    }

    fun isRightMainHand(): Boolean {
        return super.metadata.getIndex(OFFSET + 3, 1.toByte()) == 1.toByte()
    }

    fun setRightMainHand(value: Boolean) {
        super.metadata.setIndex(OFFSET + 3, Metadata.Byte(if (value) 1.toByte() else 0.toByte()))
    }

    fun getLeftShoulderEntityData(): NBT? {
        return super.metadata.getIndex<NBT>(OFFSET + 4, null)
    }

    fun setLeftShoulderEntityData(value: NBT?) {
        super.metadata.setIndex(OFFSET + 4, Metadata.NBT(value))
    }

    fun getRightShoulderEntityData(): NBT? {
        return super.metadata.getIndex<NBT>(OFFSET + 5, null)
    }

    fun setRightShoulderEntityData(value: NBT?) {
        super.metadata.setIndex(OFFSET + 5, Metadata.NBT(value))
    }

    companion object {
        val OFFSET: Byte = LivingEntityMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 1).toByte()
        private const val CAPE_BIT: Byte = 0x01
        private const val JACKET_BIT: Byte = 0x02
        private const val LEFT_SLEEVE_BIT: Byte = 0x04
        private const val RIGHT_SLEEVE_BIT: Byte = 0x08
        private const val LEFT_LEG_BIT: Byte = 0x10
        private const val RIGHT_LEG_BIT: Byte = 0x20
        private const val HAT_BIT: Byte = 0x40
    }
}