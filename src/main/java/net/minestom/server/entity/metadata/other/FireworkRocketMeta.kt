...
class FireworkRocketMeta(entity: Entity, metadata: Metadata) : EntityMeta(entity, metadata), ProjectileMeta {
    private var shooter: Entity? = null
    fun getFireworkInfo(): ItemStack {
        return super.metadata.getIndex(OFFSET.toInt(), ItemStack.AIR)
    }

    fun setFireworkInfo(value: ItemStack) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.Slot(value))
    }
    ...
    override fun setShooter(value: Entity?) {
        shooter = value
    ...
    fun setShotAtAngle(value: Boolean) {
