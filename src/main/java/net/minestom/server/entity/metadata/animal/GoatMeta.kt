...
class GoatMeta(entity: Entity, metadata: Metadata) : AnimalMeta(entity, metadata) {
    fun isScreaming(): Boolean {
        return metadata.getIndex(OFFSET.toInt(), false)
    }

    fun setScreaming(screaming: Boolean) {
        metadata.setIndex(OFFSET.toInt(), Metadata.Boolean(screaming))
    }
