...
open class VillagerMeta(entity: Entity, metadata: Metadata) : AbstractVillagerMeta(entity, metadata) {
    fun getVillagerData(): VillagerData {
        val data = super.metadata.getIndex<IntArray>(OFFSET.toInt(), null)
            ?: return VillagerData(
                Type.PLAINS,
                Profession.NONE,
                Level.NOVICE
            )
        return VillagerData(Type.VALUES[data[0]], Profession.VALUES[data[1]], Level.VALUES[data[2] - 1])
    }

    fun setVillagerData(data: VillagerData) {
        super.metadata.setIndex(
            OFFSET.toInt(), Metadata.VillagerData(
                data.type.ordinal,
    ...
    class VillagerData(private var type: Type, private var profession: Profession, private var level: Level) {
        fun getType(): Type {
            return type
        }
    ...
    enum class Profession {
        NONE, ARMORER, BUTCHER, CARTOGRAPHER, CLERIC, FARMER, FISHERMAN, FLETCHER, LEATHERWORKER, LIBRARIAN, NITWIT, UNEMPLOYED, MASON, SHEPHERD, TOOLSMITH, WEAPONSMITH;
    ...
    companion object {
        val OFFSET: Byte = AbstractVillagerMeta.Companion.MAX_OFFSET
