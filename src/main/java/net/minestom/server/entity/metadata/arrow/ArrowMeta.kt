...
class ArrowMeta(entity: Entity, metadata: Metadata) : AbstractArrowMeta(entity, metadata), ObjectDataProvider,
    ProjectileMeta {
    override var shooter: Entity? = null
    var color: Int
        get() = super.metadata.getIndex(OFFSET.toInt(), -1)
        set(value) {
            super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(value))
        }
    override val objectData: Int
        get() = if (shooter == null) 0 else shooter!!.entityId + 1
    ...
    companion object {
        val OFFSET: Byte = AbstractArrowMeta.Companion.MAX_OFFSET
