...
open class ItemContainingMeta protected constructor(entity: Entity, metadata: Metadata, defaultItemMaterial: Material) :
    EntityMeta(entity, metadata) {
    private val defaultItem: ItemStack

    init {
        defaultItem = ItemStack.of(defaultItemMaterial)
    }

    var item: ItemStack
        get() = super.metadata.getIndex(OFFSET.toInt(), defaultItem)
        set(item) {
            super.metadata.setIndex(OFFSET.toInt(), Metadata.Slot(item))
        }
