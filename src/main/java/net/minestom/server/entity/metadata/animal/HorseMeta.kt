...
class HorseMeta(entity: Entity, metadata: Metadata) : AbstractHorseMeta(entity, metadata) {
    fun getVariant(): Variant {
        return getVariantFromID(super.metadata.getIndex(OFFSET.toInt(), 0))
    }

    fun setVariant(variant: Variant) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(getVariantID(variant.marking, variant.color)))
    }

    class Variant(private var marking: Marking, private var color: Color) {
        fun getMarking(): Marking {
            return marking
        }
    ...
    companion object {
        val OFFSET: Byte = AbstractHorseMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 1).toByte()
        fun getVariantID(marking: Marking, color: Color): Int {
            return (marking.ordinal shl 8) + color.ordinal
        }

        fun getVariantFromID(variantID: Int): Variant {
            return Variant(
                Marking.VALUES[variantID shr 8],
                Color.VALUES[variantID and 0xFF]
            )
        }
    }
}