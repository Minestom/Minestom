...
open class LivingEntityMeta protected constructor(entity: Entity, metadata: Metadata) : EntityMeta(entity, metadata) {
    var isHandActive: Boolean
        get() = getMaskBit(OFFSET.toInt(), IS_HAND_ACTIVE_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), IS_HAND_ACTIVE_BIT, value)
        }
    var activeHand: Hand
        get() = if (getMaskBit(OFFSET.toInt(), ACTIVE_HAND_BIT)) Hand.OFF else Hand.MAIN
        set(hand) {
            setMaskBit(OFFSET.toInt(), ACTIVE_HAND_BIT, hand == Hand.OFF)
        }
    var isInRiptideSpinAttack: Boolean
        get() = getMaskBit(OFFSET.toInt(), IS_IN_SPIN_ATTACK_BIT)
        set(value) {
            setMaskBit(OFFSET.toInt(), IS_IN_SPIN_ATTACK_BIT, value)
        }
    var health: Float
        get() = super.metadata.getIndex(OFFSET + 1, 1f)
        set(value) {
            super.metadata.setIndex(OFFSET + 1, Metadata.Float(value))
        }
    var potionEffectColor: Int
        get() = super.metadata.getIndex(OFFSET + 2, 0)
        set(value) {
            super.metadata.setIndex(OFFSET + 2, Metadata.VarInt(value))
        }
    var isPotionEffectAmbient: Boolean
        get() = super.metadata.getIndex(OFFSET + 3, false)
        set(value) {
            super.metadata.setIndex(OFFSET + 3, Metadata.Boolean(value))
        }
    var arrowCount: Int
        get() = super.metadata.getIndex(OFFSET + 4, 0)
        set(value) {
            super.metadata.setIndex(OFFSET + 4, Metadata.VarInt(value))
        }
    var healthAddedByAbsorption: Int
        get() = super.metadata.getIndex(OFFSET + 5, 0)
        set(value) {
            super.metadata.setIndex(OFFSET + 5, Metadata.VarInt(value))
        }
    var bedInWhichSleepingPosition: Point?
        get() = super.metadata.getIndex<Point>(OFFSET + 6, null)
        set(value) {
    ...
    companion object {
        ...
        private const val IS_HAND_ACTIVE_BIT: Byte = 0x01
        private const val ACTIVE_HAND_BIT: Byte = 0x02
        private const val IS_IN_SPIN_ATTACK_BIT: Byte = 0x04
    }
}