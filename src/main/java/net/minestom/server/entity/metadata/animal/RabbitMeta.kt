...
class RabbitMeta(entity: Entity, metadata: Metadata) : AnimalMeta(entity, metadata) {
    fun getType(): Type {
        val id = super.metadata.getIndex(OFFSET.toInt(), 0)
        return if (id == 99) {
            Type.KILLER_BUNNY
        } else Type.VALUES[id]
    }
    ...
    fun setType(value: Type) {
        val id = if (value == Type.KILLER_BUNNY) 99 else value.ordinal
        super.metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(id))
    }

    enum class Type {
        BROWN, WHITE, BLACK, BLACK_AND_WHITE, GOLD, SALT_AND_PEPPER, KILLER_BUNNY;
