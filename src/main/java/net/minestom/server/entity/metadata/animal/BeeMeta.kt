...
class BeeMeta(entity: Entity, metadata: Metadata) : AnimalMeta(entity, metadata) {
    fun isAngry(): Boolean {
        return getMaskBit(OFFSET.toInt(), ANGRY_BIT)
    }

    fun setAngry(value: Boolean) {
        setMaskBit(OFFSET.toInt(), ANGRY_BIT, value)
    }

    fun isHasStung(): Boolean {
        return getMaskBit(OFFSET.toInt(), HAS_STUNG_BIT)
    }

    fun setHasStung(value: Boolean) {
        setMaskBit(OFFSET.toInt(), HAS_STUNG_BIT, value)
    }

    fun isHasNectar(): Boolean {
        return getMaskBit(OFFSET.toInt(), HAS_NECTAR_BIT)
    }

    fun setHasNectar(value: Boolean) {
        setMaskBit(OFFSET.toInt(), HAS_NECTAR_BIT, value)
    }
    ...
    companion object {
        ...
        private const val ANGRY_BIT: Byte = 0x02
        private const val HAS_STUNG_BIT: Byte = 0x04
        private const val HAS_NECTAR_BIT: Byte = 0x08
    }
}