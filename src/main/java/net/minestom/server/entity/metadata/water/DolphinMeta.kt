...
class DolphinMeta(entity: Entity, metadata: Metadata) : WaterAnimalMeta(entity, metadata) {
    fun getTreasurePosition(): Point {
        return super.metadata.getIndex(OFFSET.toInt(), Vec.ZERO)
    }

    fun setTreasurePosition(value: Point) {
        super.metadata.setIndex(OFFSET.toInt(), Metadata.Position(value))
    }

    fun isCanFindTreasure(): Boolean {
        return super.metadata.getIndex(OFFSET + 1, false)
    }

    fun setCanFindTreasure(value: Boolean) {
    ...
    companion object {
        val OFFSET: Byte = WaterAnimalMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 3).toByte()
    }
}