...
class BatMeta(entity: Entity, metadata: Metadata) : AmbientCreatureMeta(entity, metadata) {
    fun isHanging(): Boolean {
        return getMaskBit(OFFSET.toInt(), IS_HANGING_BIT)
    }

    fun setHanging(value: Boolean) {
        setMaskBit(OFFSET.toInt(), IS_HANGING_BIT, value)
    }

    companion object {
        val OFFSET: Byte = AmbientCreatureMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 0).toByte()
        private const val IS_HANGING_BIT: Byte = 0x01
    }
}