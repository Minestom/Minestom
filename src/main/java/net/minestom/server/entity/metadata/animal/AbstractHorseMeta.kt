...
open class AbstractHorseMeta protected constructor(entity: Entity, metadata: Metadata) : AnimalMeta(entity, metadata) {
    fun isTamed(): Boolean {
        return getMaskBit(OFFSET.toInt(), TAMED_BIT)
    }

    fun setTamed(value: Boolean) {
        setMaskBit(OFFSET.toInt(), TAMED_BIT, value)
    }

    fun isSaddled(): Boolean {
        return getMaskBit(OFFSET.toInt(), SADDLED_BIT)
    }

    fun setSaddled(value: Boolean) {
        setMaskBit(OFFSET.toInt(), SADDLED_BIT, value)
    }

    fun isHasBred(): Boolean {
        return getMaskBit(OFFSET.toInt(), HAS_BRED_BIT)
    }

    fun setHasBred(value: Boolean) {
        setMaskBit(OFFSET.toInt(), HAS_BRED_BIT, value)
    }

    fun isEating(): Boolean {
        return getMaskBit(OFFSET.toInt(), EATING_BIT)
    }

    fun setEating(value: Boolean) {
        setMaskBit(OFFSET.toInt(), EATING_BIT, value)
    }

    fun isRearing(): Boolean {
        return getMaskBit(OFFSET.toInt(), REARING_BIT)
    }

    fun setRearing(value: Boolean) {
        setMaskBit(OFFSET.toInt(), REARING_BIT, value)
    }

    fun isMouthOpen(): Boolean {
        return getMaskBit(OFFSET.toInt(), MOUTH_OPEN_BIT)
    }

    fun setMouthOpen(value: Boolean) {
        setMaskBit(OFFSET.toInt(), MOUTH_OPEN_BIT, value)
    }

    fun getOwner(): UUID {
        return super.metadata.getIndex<UUID>(OFFSET + 1, null)
    }

    fun setOwner(value: UUID?) {
        super.metadata.setIndex(OFFSET + 1, Metadata.OptUUID(value))
    }

    companion object {
        val OFFSET: Byte = AnimalMeta.Companion.MAX_OFFSET
        val MAX_OFFSET = (OFFSET + 2).toByte()
        private const val TAMED_BIT: Byte = 0x02
        private const val SADDLED_BIT: Byte = 0x04
        private const val HAS_BRED_BIT: Byte = 0x08
        private const val EATING_BIT: Byte = 0x10
        private const val REARING_BIT: Byte = 0x20
        private const val MOUTH_OPEN_BIT: Byte = 0x40
    }
}