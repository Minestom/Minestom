...
class AxolotlMeta(entity: Entity, metadata: Metadata) : AnimalMeta(entity, metadata) {
    fun getVariant(): Variant {
        return Variant.VALUES[super.metadata.getIndex(OFFSET.toInt(), 0)]
    }

    fun setVariant(variant: Variant) {
        metadata.setIndex(OFFSET.toInt(), Metadata.VarInt(variant.ordinal))
    }

    fun isPlayingDead(): Boolean {
        return metadata.getIndex(OFFSET + 1, false)
    }

    fun setPlayingDead(playingDead: Boolean) {
    ...
    fun setFromBucket(fromBucket: Boolean) {
