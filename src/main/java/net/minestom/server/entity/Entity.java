...
public class Entity implements Viewable, Tickable, Schedulable, Snapshotable, EventHandler<EntityEvent>, Taggable,
    ...
    public void refreshPosition(@NotNull final Pos newPosition, boolean ignoreView, boolean sendPackets) {
        ...
        if (distanceX > 8 || distanceY > 8 || distanceZ > 8) {
            PacketViewableUtils.preparePacket(chunk, new EntityTeleportPacket(getEntityId(), position, isOnGround()), this);
            nextSynchronizationTick = synchronizationTicks + 1;
        } else if (positionChange && viewChange) {
            PacketViewableUtils.preparePacket(chunk, EntityPositionAndRotationPacket.getPacket(getEntityId(), position,
                    lastSyncedPosition, isOnGround()), this);
            // Fix head rotation
            PacketViewableUtils.preparePacket(chunk, new EntityHeadLookPacket(getEntityId(), position.yaw()), this);
    ...
    protected void synchronizePosition() {
        ...
        PacketViewableUtils.preparePacket(currentChunk, new EntityTeleportPacket(getEntityId(), posCache, isOnGround()), this);
        if (posCache.yaw() != lastSyncedPosition.yaw()) {
            PacketViewableUtils.preparePacket(currentChunk, new EntityHeadLookPacket(getEntityId(), position.yaw()), this);
        }
