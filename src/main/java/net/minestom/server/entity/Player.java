...
/**
 * Those are the major actors of the server, they are not necessary backed by a {@link
 * NettyPlayerConnection} as shown by {@link FakePlayer}.
 *
 * <p>You can easily create your own implementation of this and use it with {@link
 * ConnectionManager#setPlayerProvider(PlayerProvider)}.
 */
public class Player extends LivingEntity implements CommandSender {
  ...
  private final PlayerInventory inventory;
                   // disabled
  private final Set<Player> targetBreakers =
      Collections.singleton(
          this); // Only used if multi player breaking is disabled, contains only this player
  // Statistics
  ...
  private byte
      targetStage; // The current stage of the target block, only if multi player breaking is
  // Position synchronization with viewers
  private long lastPlayerSynchronizationTime;
  private float lastPlayerSyncX,
      lastPlayerSyncY,
      lastPlayerSyncZ,
      lastPlayerSyncYaw,
      lastPlayerSyncPitch;
   ...
   * every 1 tick + (viewers/{@code playerSynchronizationGroup}). (eg with a value of 100, having
   * 300 viewers means sending the synchronization packet every 3 ticks)
   *
   * <p>Used to prevent sending exponentially more packets and therefore reduce network load.
   *
   ...
   *
   * <p>WARNING: executed in the main update thread
   */
  protected void init() {
  ...
  public void update(long time) {
    ...
    if (targetCustomBlock != null) {
      ...
      final boolean processStage =
          targetBreakDelay < 0 || targetBlockBreakCount >= targetBreakDelay;
      ...
      if (processStage) {
        ...
        if (targetCustomBlock.enableMultiPlayerBreaking()) {
          ...
          final boolean canContinue =
              this.targetCustomBlock.processStage(
                  instance, targetBlockPosition, this, stageIncrease);
          if (canContinue) {
            final Set<Player> breakers =
                targetCustomBlock.getBreakers(instance, targetBlockPosition);
        ...
        if (entity instanceof ExperienceOrb) {
          ...
          if (expandedBoundingBox.intersect(itemBoundingBox)) {
            ...
            callCancellableEvent(
                ...
                () -> {
                  short experienceCount =
                      pickupExperienceEvent.getExperienceCount(); // TODO give to player
                  entity.remove();
                });
          }
        }
  ...
  public boolean removeViewer(@NotNull Player player) {
    ...
    if (this.getTeam() != null
        && this.getTeam().getMembers().size() == 1) // If team only contains "this" player
   ...
   *
   * <p>Be aware that because chunk operations are expensive, it is possible for this method to be
  ...
  /**
   * Changes the player instance without changing its position (defaulted to {@link
   * #getRespawnPoint()} if the player is not in any instance.
   *
   * @param instance the new player instance
   ...
   *
   * <p>Does add the player to {@code instance}, remove all viewable entities and call {@link
   * PlayerSpawnEvent}.
   *
   * <p>UNSAFE: only called with {@link #setInstance(Instance, Position)}.
   *
   ...
   *
   * @param message the message to send, you can use {@link ColoredText} and/or {@link RichMessage}
   *     to create it easily
   */
   ...
   */
  public void playSound(
      @NotNull Sound sound, @NotNull SoundCategory soundCategory, float volume, float pitch) {
  ...
  public void playEffect(
      @NotNull Effects effect, int x, int y, int z, int data, boolean disableRelativeVolume) {
  ...
  public void sendHeaderFooter(@Nullable ColoredText header, @Nullable ColoredText footer) {
    PlayerListHeaderAndFooterPacket playerListHeaderAndFooterPacket =
        new PlayerListHeaderAndFooterPacket();
  ...
  public void setFoodSaturation(float foodSaturation) {
    Check.argCondition(
        !MathUtils.isBetween(foodSaturation, 0, 5), "Food saturation has to be between 0 and 5");
   ...
   *
   * <p>Sets to null to show the player username.
   *
   ...
   *
   * <p>This does remove the player for all viewers to spawn it again with the correct new skin.
   *
   ...
   *
   * <p>Returns false if {@code item} is air.
   *
   ...
   *
   * <p>Can be altered by the {@link PlayerRespawnEvent#setRespawnPosition(Position)}.
   *
   ...
   *
   * <p>It does remove and add the player from the chunks viewers list when removed or added. It
  ...
  public void refreshVisibleEntities(@NotNull Chunk newChunk) {
    ...
    final long[] chunksInRange =
        ChunkUtils.getChunksInRange(newChunk.toPosition(), entityViewDistance);
  ...
  public void teleport(
      @NotNull Position position, @Nullable long[] chunks, @Nullable Runnable callback) {
    super.teleport(
        position,
        chunks,
        () -> {
  ...
  public void teleport(@NotNull Position position, @Nullable Runnable callback) {
    ...
    final long[] chunks = sameChunk ? null : ChunkUtils.getChunksInRange(position, getChunkRange());
   ...
   *
   * <p>Used to send packets and get stuff related to the connection.
   *
   ...
   *
   * <p>WARNING: this can be bypassed by hacked client, this is only what the client told the
   * server.
   *
   ...
   *
   * <p>Should only be used within an inventory listener (event or condition).
   *
   ...
   *
   * <p>Shouldn't be used externally without proper understanding of its consequence.
   *
   ...
   *
   * <p>WARNING: adding or removing a chunk there will not load/unload it, use {@link
  ...
  protected void updatePlayerPosition() {
    ...
    positionAndLookPacket.position =
        position.clone(); // clone needed to prevent synchronization issue
  ...
  public void setPermissionLevel(int permissionLevel) {
    Check.argCondition(
        !MathUtils.isBetween(permissionLevel, 0, 4), "permissionLevel has to be between 0 and 4");
  ...
  /**
   * This do update the {@code invulnerable} field in the packet {@link PlayerAbilitiesPacket} and
   * prevent the player from receiving damage.
   *
   ...
   *
   * <p>Mostly unsafe since there is nothing to backup the value, used internally for creative
   * players.
   *
  ...
  /**
   * Changes the player ability "Creative Mode". <a
   * href="https://wiki.vg/Protocol#Player_Abilities_.28clientbound.29">see</a>
   *
   * <p>WARNING: this has nothing to do with {@link CustomBlock#getBreakDelay(Player, BlockPosition,
   * byte, Set)}.
   *
  ...
  /**
   * Sends to the player a {@link PlayerAbilitiesPacket} with all the updated fields (walkingSpeed
   * set to 0.1).
   */
  ...
  /**
   * All packets in the queue are executed in the {@link #update(long)} method It is used internally
   * to add all received packet from the client. Could be used to "simulate" a received packet, but
  ...
  public void refreshLatency(int latency) {
    ...
    PlayerInfoPacket playerInfoPacket =
        new PlayerInfoPacket(PlayerInfoPacket.Action.UPDATE_LATENCY);
   ...
   *
   * <p>Warning: could lead to have the player kicked because of a wrong keep alive packet.
   *
   ...
   * true.
   *
   * <p>Warning: the player will not be noticed by this chance, only his viewers, see instead:
  ...
  private void refreshBreakDelay(@Nullable Set<Player> breakers) {
    ...
    final byte stage =
        multiPlayerBreaking
            ? targetCustomBlock.getBreakStage(instance, targetBlockPosition)
            : targetStage;
    // Retrieve the break delay for the current stage
    this.targetBreakDelay =
        targetCustomBlock.getBreakDelay(this, targetBlockPosition, stage, breakers);
  }

  /**
   * Resets data from the current block the player is mining. If the currently mined block (or if
   * there isn't any) is not a {@link CustomBlock}, nothing happen.
   */
  ...
  /**
   * @return the chunk range of the viewers, which is {@link MinecraftServer#getChunkViewDistance()}
   *     or {@link PlayerSettings#getViewDistance()} based on which one is the lowest
   */
  ...
  /**
   * Sends all the related packet to have the player sent to another with related data (create
   * player, spawn position, velocity, metadata, equipments, passengers, team).
   *
   * <p>WARNING: this alone does not sync the player, please use {@link #addViewer(Player)}.
   *
  ...
  public class PlayerSettings {
     ...
     *
     * <p>WARNING: the player will not be noticed by this change, probably unsafe.
     *
