...
public final class GeneratorImpl {
    ...
    public static UnitImpl area(DynamicRegistry<Biome> biomeRegistry, BlockVec start, int width, int height, int depth, GenSection[] areaSections) {
        ...
        for (int i = 0; i < sectionCount; i++) {
            ...
            final BlockVec point = to3D(i, width, height);
    ...
    static final class DynamicFork implements Block.Setter {
        ...
        public void setBlockArea(Area area, Block block) {
            ...
            Fork fork = cachedFork;
            for (BlockVec vec : area)
                fork = setBlock(fork, vec.blockX(), vec.blockY(), vec.blockZ(), block);
            if (fork != cachedFork) {
                this.fork = fork;
            }
        }
        ...
        private Fork resize(@Nullable Fork fork, int x, int y, int z) {
            ...
            for (int i = 0; i < sectionCount; i++) {
                if (newSections[i] == null) {
                    final BlockVec coordinates = to3D(i, newWidth, newHeight);
    ...
    private static BlockVec to3D(int idx, int width, int height) {
